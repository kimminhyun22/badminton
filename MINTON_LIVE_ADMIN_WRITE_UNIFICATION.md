...
## 실측에서 나온 것 (2026-08-04, v1.10.513)

실제 Firebase 로 관리자 동작을 하나씩 눌러 본 기록입니다. **단위 테스트가 못 잡은
결함이 다섯 개 나왔습니다.** 엔진만 직접 호출하는 테스트로는 부족합니다.

| 증상 | 원인 |
|---|---|
| 모든 관리자 명령이 "운영 권한이 있는 회원만…" 으로 거절 | 서버 `validateCommon` 만 넓히고 **클라이언트 `_dailyOfficialRequestError` 를 안 고침**. 서버가 적용한 결과를 관리자 원본이 다시 검사해 거절 |
| 새 명령 11종이 "지원하지 않는 요청" | 같은 함수의 마지막 줄로 떨어짐. 명령을 만들 때 클라이언트 목록에 추가하지 않음 |
| "종료할 진행중 경기를 찾지 못했습니다" | 자동 배정은 명령을 안 거치고 로컬에서 경기를 시작합니다. 그 경기는 게시로만 서버에 올라가는데, **자기 게시가 끝나기 전에 명령을 보내는 경합** |
| 파트너 해제·게임신청 취소가 전부 실패 | 서버가 대조하는 `expectedPlayerIds` 를 안 보냄 |
| 고쳤는데도 옛 코드가 계속 돎 | `index.html` 이 `js/daily.js` 를 **버전 없이** 로드. 브라우저가 캐시를 유지 |

### 여기서 얻은 규칙

1. **같은 검사가 두 벌 있으면 한쪽만 고치지 않는다.** 서버의 관리자 전용 명령
   목록을 읽어 클라이언트가 아는지 대조하는 회귀를 넣었습니다
2. **명령을 보내기 전에 관리자 원본을 먼저 게시한다.** `_dailyCheckinNeedsPublish`
   플래그로 가르면 안 됩니다 — 무관한 게시가 끝나도 꺼져서 경합을 못 막습니다
3. **화면이 부르는 js·css 에는 전부 `?v=` 를 단다.** 회귀로 고정했습니다
4. **실기기 확인은 로컬 서버 말고 실제 배포로.** 인증서·서비스워커·포트로 몇 시간을
   버렸습니다. 실제 배포는 진짜 HTTPS 라 그 전부가 없습니다

## 팀전 점검 (2026-08-04)

민턴LIVE 와 같은 구조로 바꾸자는 검토를 했고, **바꾸지 않기로** 했습니다.

팀전은 Cloud Function 을 안 씁니다. 관리자가 `live/<id>` 를 통째 update 하되
`matches` 는 제외하고, 회원이 쓰는 `resultInputs` 는 리스너로 읽어 되돌려 씁니다.
민턴LIVE 를 터뜨린 "통째 쓰기 ↔ 부분 수정" 충돌이 성립하지 않고 사고 이력도
없습니다. 명령 인프라를 새로 만들 이유를 찾지 못했습니다.

대신 운영자 요구(임원이 단장·부단장을 돕는다)에 맞춰 둘을 고쳤습니다.

**출결 권한** — `toggleMemberLate` 에 권한 검사가 **아예 없어** 누구나 남의 지각을
켜고 끌 수 있었습니다. 지각·도착 확인은 단장·부단장·클럽 임원·운영 도우미만,
뒷풀이는 본인(또는 운영진 대행)으로 좁혔습니다. 버튼만 감추지 않고 저장 함수
안에서 막습니다. 용어도 '늦음' → '지각' 으로 민턴LIVE 에 맞췄습니다.

**화면 순서** — 누구에게나 본인 이름표가 맨 위, 출결을 처리하는 팀 명단이
7번째였습니다. 운영진에게는 운영 현황 → 경기(승패 입력) → 팀 명단(출결) →
내 카드 순으로 바꿨습니다. 민턴LIVE 의 `placeEventPanelForViewer` 와 같은 원칙입니다.

**승패 입력은 좁히지 않았습니다.** 그 코트에서 뛴 넷이 가장 정확히 알고, 임원
한 명이 네 코트를 돌 수 없습니다. 이미 `resultConflicts` 로 불일치가 기록되어
임원이 최종 판단할 수 있습니다. 이 결정을 회귀에 못 박아 뒀습니다.

## 아직 안 한 것

- **`firebase deploy --only functions`** — 경기 취소 후 자동 투입 제외가 서버에
  안 올라갔습니다. 이걸 안 하면 취소한 코트에 서버가 새 대진을 넣어 갈라집니다
- **실측 미완료 4종** — 도착 전 등록(모달 경로) · 수동 경기 등록 · 다음 대진 투입 ·
  대진 선수/순서 변경. 코드와 단위 테스트로만 확인했습니다
- **`database.rules.json`** — 세션 id 만 알면 인증 없이 세션 전체를 읽고 쓸 수
  있습니다. 이번 작업으로 생긴 문제는 아니지만, 명령 경로를 아무리 조여도 이
  구멍이 열려 있으면 의미가 반감됩니다
