...
const BALANCE_TEAM_DIFF_TARGET=MATCH_QUALITY?.constants.teamDiffTarget??1.5;
const BALANCE_TEAM_DIFF_LIMIT=MATCH_QUALITY?.constants.teamDiffLimit??2;
const BALANCE_TEAM_DIFF_SEVERE=MATCH_QUALITY?.constants.teamDiffSevere??3;
...
function switchNav(p){
  document.querySelectorAll('.nav-tab').forEach((t,i)=>t.classList.toggle('active',['main','roster','manual'][i]===p));
  ['pageMain','pageRoster','pageManual'].forEach((id,i)=>document.getElementById(id).classList.toggle('active',['main','roster','manual'][i]===p));
}
...
function applyColumnTeamAssignment(valid){
  const blue=valid.filter(p=>p.team==='청팀');
  const white=valid.filter(p=>p.team==='홍팀');
  // those without team go to smaller team
  valid.filter(p=>p.team!=='청팀'&&p.team!=='홍팀').forEach(p=>{
    if(blue.length<=white.length){p.team='청팀';blue.push(p);}
    else{p.team='홍팀';white.push(p);}
  });
  ...
  btn.innerHTML='✅ 시트 팀 배정 적용됨 (재배정하려면 클릭)';
}
...
function _teamSyncMatchModeSession(){
  if(typeof rsvpPushSession==='function')rsvpPushSession();
}
...
function updateSettingsMiniSummary(){
  const el=document.getElementById('settingsMiniSummary');
  if(!el)return;
...
function generate(opts={}){
  ...
  if(useFixedTeams&&teamAssignment){
    ...
    participants.forEach(p=>{
      if(bn.has(p.name))p.team='청팀';
      else if(wn.has(p.name))p.team='홍팀';
      else p.team='';
    });
  } else {
  ...
  while(matches.length<totalMatches){
    ...
    for(let pass=1;pass<=2;pass++){
      for(let court=1;court<=settings.courts;court++){
        ...
        if(!match)continue;
        if(_matchStructureErrorCount(match,settings)>0)continue;
...
function fillMissingGames(participants,settings,matches,maxMatches=Infinity){
  ...
  while(_forceSafety-- > 0) {
    ...
    for(let court=1; court<=courts; court++) {
      ...
      if(new Set(_four.map(p=>p.name)).size<4) continue;

      // 모든 타입으로 formTeams 시도
      let _match = null;
      const _modeTypes=settings.teamMode
        ?[[true,'any'],[true,'adjust'],[true,'men'],[true,'women']]
        :[[false,'any'],[false,'men'],[false,'women']];
      for(const [_tm,_type] of _modeTypes) {
        _match = formTeams(_four, _tm, _type, 99);
        if(_match) break;
      }
      if(!_match) continue;
      if(_matchStructureErrorCount(_match,settings)>0) continue;
...
function selectFourFreeMode(pool,type,maxLD){
  ...
  for(let i=0;i<n-3;i++)for(let j=i+1;j<n-2;j++)for(let k=j+1;k<n-1;k++)for(let l=k+1;l<n;l++){
    ...
    const partnerOk=four.every(p=>{
      if(!p.partnerName) return true;
      return four.some(x=>x.name===p.partnerName);
    });
...
function _matchStructureErrorCount(m,settings){
  if(!m)return 1;
  const players=[m.team1A,m.team1B,m.team2C,m.team2D];
  if(players.some(p=>!p||!p.name))return 1;
  if(new Set(players.map(p=>p.name)).size!==4)return 1;
  if(settings?.teamMode){
    if(m.team1A.team!=='청팀'||m.team1B.team!=='청팀')return 1;
    if(m.team2C.team!=='홍팀'||m.team2D.team!=='홍팀')return 1;
  }
  return 0;
}
...
function formTeams(four,teamMode,type,maxLD,allowPartnerSplit){
  ...
  for(const combo of combos){
    ...
    if(!p1pair) score+=partnerRepeatPenalty(t1[0].partnerCount[t1[1].name]||0);
    if(!p2pair) score+=partnerRepeatPenalty(t2[0].partnerCount[t2[1].name]||0);
...
function _repairParticipation(matches,participants,settings,historyMatches=[]){
  ...
  while(safety-->0){
    ...
    matches.forEach((m,mi)=>{
      ...
      const roundNames=new Set(matches
        .filter((x,xi)=>xi!==mi&&x.round===m.round)
        .flatMap(x=>slots.map(k=>x[k].name)));
      if(names.has(p.name))return;
      if(roundNames.has(p.name))return;
      slots.forEach(slot=>{
        const q=m[slot];
...
function _bracketQualityScore(matches, participants, settings){
  ...
  const partner3Plus=pcVals.filter(c=>c>=3).length;
...
function _autoSearchTries(playerCount,hasNewJoiner=false){
...
function clickWin(idx,side){
  ...
  if(winOverride[idx]) liveWinAt[idx]=Date.now();
  else delete liveWinAt[idx];
...
function _teamStoredLiveId(){
  try{return localStorage.getItem(TEAM_LIVE_STORAGE_KEY)||localStorage.getItem(LEGACY_LIVE_STORAGE_KEY)||'';}catch(e){return '';}
}
function _teamSaveLiveId(id){
  try{
    if(id)localStorage.setItem(TEAM_LIVE_STORAGE_KEY,id);
    localStorage.removeItem(LEGACY_LIVE_STORAGE_KEY);
  }catch(e){}
}
function _teamClearStoredLiveId(id){
  try{
    const teamId=localStorage.getItem(TEAM_LIVE_STORAGE_KEY);
    if(!id||teamId===id)localStorage.removeItem(TEAM_LIVE_STORAGE_KEY);
...
function _buildLiveState(){
  ...
  const matches=currentMatches.map((m,i)=>{
    ...
    return {
      ...
      win,
      winAt: win ? (liveWinAt[i]||Date.now()) : null
    };
  });
  const ordered=matches.map((m,i)=>({m,i})).sort((a,b)=>(a.m.round-b.m.round)||(a.m.court-b.m.court));
  ordered.forEach(({m},pos)=>{
    if(!m.winAt) return;
    const next=ordered.slice(pos+1).find(x=>!x.m.win && x.m.court===m.court);
    if(next && !next.m.startAt) next.m.startAt=m.winAt;
  });
  let bW=0,wW=0;
  if(isTeam){
...
function _liveHas(map,name){
  return !!(map && map[_liveKey(name)]);
}
function _duplicateNames(list=currentParticipants){
  const counts={};
  (list||[]).forEach(p=>{
...
function _liveNamesPreview(names,max=5){
  if(!names||!names.length) return '없음';
  const shown=names.slice(0,max).map(esc).join(', ');
  return shown+(names.length>max?` 외 ${names.length-max}명`:'');
}
function _upcomingLiveNames(){
  if(!currentMatches.length) return [];
  const courtCount=Math.max(1,parseInt(currentSettings?.courts||document.getElementById('courts')?.value||'4',10)||4);
  const pending=currentMatches.map((m,i)=>({m,i}))
    .filter(x=>!_isMatchDone(x.i))
    .sort((a,b)=>(a.m.round-b.m.round)||(a.m.court-b.m.court))
    .slice(0,courtCount);
  const seen=new Set(), names=[];
  pending.forEach(({m})=>[m.team1A,m.team1B,m.team2C,m.team2D].forEach(p=>{
    const n=p&&p.name;
...
function _renderLiveOpsSummary(){
  ...
  if(!el) return;
  if(!_liveOn || !currentParticipants.length){
  ...
  el.innerHTML=`
    ...
    <div class="live-ops-card party">
      ...
      <div class="live-ops-v">${partyNames.length}명</div>
      <div class="live-ops-names">${_liveNamesPreview(partyNames)}</div>
    </div>
    ${conflictCount?`<div class="live-ops-card warn">
...
function _syncLiveWinsFromData(data){
  ...
  currentMatches.forEach((m,i)=>{
    ...
    if(next!==cur){
      if(next) winOverride[i]=next;
      else delete winOverride[i];
      changed=true;
    }
    if(next && fbWinAtMap[key]) liveWinAt[i]=fbWinAtMap[key];
    if(!next) delete liveWinAt[i];
  });
...
이어서 중계를 재개할까요?`);
    if(shouldResume){
      ...
      const fbWinMap={};
      const fbWinAtMap={};
      fbMatches.forEach(fm=>{
        if(fm.win) fbWinMap[`${fm.round}_${fm.court}`]=fm.win;
        if(fm.winAt) fbWinAtMap[`${fm.round}_${fm.court}`]=fm.winAt;
      });
      currentMatches.forEach((m,i)=>{
        const key=`${m.round||0}_${m.court||0}`;
        if(fbWinMap[key]) winOverride[i]=fbWinMap[key];
        if(fbWinAtMap[key]) liveWinAt[i]=fbWinAtMap[key];
      });
      if(currentMatches.length) renderResults(currentMatches,currentParticipants,currentSettings);
...
async function _teamClearLiveBroadcastData(explicitLiveId){
  ...
  _teamResetLocalLiveState(liveId);
  if(db&&liveId){
    await db.ref('live/'+liveId).remove().catch(()=>{});
  }
}
...
function updateCurrentRoundHighlight(){
  ...
  if(cur!=null){
    ...
    if(goBtn){ goBtn.style.display='flex'; goBtn.querySelector('.gcb-round').textContent='R'+cur; }
  } else {
    // 전부 완료
...
function _qualityAssessment(matches,participants,settings){
  ...
  return {gpp,avgLD,spikes,maxLD,balanceCautionCount:balance.cautionCount,
    ...
    repeatedPartnerPairs,maxPartnerRepeat,
  ...
  const bracketHtml=Object.keys(byRound).sort((a,b)=>a-b).map(r=>{
    ...
    const cards=ms.map(m=>{
      ...
      return `<div class="match-card ${tc}${m.isFiller?' filler':''}" id="mc_${idx}">
        <div class="match-top">
          <span class="court-badge">${m.court}코트</span>
    ...
    return `<div class="round-block${isCurrent?' round-current':''}" id="roundBlock_${r}"><div class="round-header"><span class="round-badge">ROUND ${r}</span>${nowBadge}${editBtn}<div class="round-line"></div></div><div class="match-grid">${cards}</div></div>`;
  }).join('');
...
function buildPrintHtml(mode, isTeam, bn, wn){
  if(mode==='bracket'){
    ...
    const roundsHtml=rounds.map(r=>{
      ...
      return `<div class="p-round-block"><div class="p-round-title">ROUND ${r}</div><div class="p-match-grid">${cards}</div></div>`;
    }).join('');
...
function buildMobileHtml(mode, isTeam, bn, wn){
  if(mode==='bracket'){
    ...
    const roundsHtml=rounds.map(r=>{
      ...
      return `<div class="render-round"><div class="render-round-badge">ROUND ${r}</div><div class="render-match-grid">${cards}</div></div>`;
    }).join('');
...
function restoreState(opts={}){
  try{
    ...
    if(state.teamAssignment){
      ...
      updateTeamModeBadge();
      renderTeamList();
    } else {teamAssignment=null;updateTeamModeBadge();}

    currentParticipants=state.participants.map(p=>Object.assign({
      _valid:true,partnerCount:{},opponentCount:{},lastRoundPlayed:0,_levelRaw:'',_genderRaw:'',isGuest:!!p.isGuest,isClubOfficial:!!p.isClubOfficial,
      adjustmentPlayed:0,
      memberId:'',club:'',
...
function alertRoundLocked(r){
  alert(`ROUND ${r}은(는) 중간 선수 투입 이후 확정되어 대진을 변경할 수 없습니다.\n\n승/패 입력은 그대로 가능합니다.\n(변경하려면 ROUND ${_lockedBeforeRound} 이상에서 선수 변경을 하세요.)`);
}
...
function openRoundChange(r){
  ...
  document.getElementById('cpInfoBox').innerHTML=
    `<b>ROUND ${r}부터</b> 선수 변경 후 재배정합니다.<br>`+
    `유지: <b>ROUND ${r} 이전 ${keepCount}게임</b> · 재생성: <b>ROUND ${r}~ ${regenCount}게임</b>${teamNote}`;
  _cpRenderPlayerList();
  _cpRenderNewList();
  _cpUpdateSummary();
...
function _genBestMatches(activeParticipants, settings, totalNewMatches, tries=5, historyMatches=[], finalQuality=false){
  ...
  // 중도 투입은 출전 이력이 서로 달라 후보별 편차가 크다.
  // 20회 안에 공정한 후보가 없을 때만 최대 60회까지 추가 탐색한다.
  const minTries=_hasNJ?Math.max(tries,20):tries;
  // 60회 안에 목표 미달이 남을 때만 추가 탐색하며, 대부분은 20회에서 종료된다.
...
function executeChangeModal(options={}){
  ...
  if(currentSettings.teamMode){
    ...
    if(b.length<2){alert('청팀 최소 2명이 필요합니다.');return;}
    if(w.length<2){alert('홍팀 최소 2명이 필요합니다.');return;}
  }
...
async function resetAll(){
  ...
  document.getElementById('teamListWrap').classList.remove('show');
  document.getElementById('teamAssignBtn').classList.remove('done','hidden');
  document.getElementById('teamAssignBtn').innerHTML='⚖️ 청/홍팀 자동 배정 <span style="font-size:.7rem;opacity:.6">(선택)</span>';
  const rbtn=document.getElementById('teamReassignBtn');
...
function _teamEnsureMemberId(player){
  if(!player)return '';
  if(!player.memberId){
    player.memberId=_rsvpMemberId({name:player.name||'',club:player.club||''});
  }
...
function _teamRosterBridge(){
  return typeof window!=='undefined' ? window.KokMatchRosterBridge || null : null;
}
function _teamSaveRosterBridge(){
  const bridge=_teamRosterBridge();
  return bridge?bridge.save('team',_directPlayers):null;
}
function _teamDailyRosterSnapshot(){
  const bridge=_teamRosterBridge();
  return bridge?bridge.load('daily'):{players:[],savedAt:0};
}
function _teamParticipantHandoffSnapshot(){
  const bridge=_teamRosterBridge();
  return bridge&&typeof bridge.loadHandoff==='function'?bridge.loadHandoff():{players:[],source:'',savedAt:0};
}
function _teamApplyParticipantProfiles(source){
...
function teamImportDailyRoster(options={}){
  if(isTeamSampleMode()){
    alert('샘플 모드에서는 선수 명단을 바꿀 수 없습니다.');
    return;
  }
  ...
  const snapshot=options.snapshot||_teamDailyRosterSnapshot();
  const source=snapshot.players||[];
  if(!source.length){
    alert('가져올 민턴LIVE 선수 명단이 없습니다.');
    return;
  }
  if(!options.silent&&_directPlayers.length&&!confirm(`현재 팀전 선수 ${_directPlayers.length}명을 지우고\n민턴LIVE 선수 ${source.length}명을 가져올까요?`))return;
  if(!options.silent&&_directPlayers.length)_captureUndoSnapshot('민턴LIVE 선수 명단 가져오기 전');
  _teamApplyParticipantProfiles(source);
  if(!options.silent)alert(`민턴LIVE 선수 ${_directPlayers.length}명을 팀전 명단으로 가져왔습니다.`);
  return true;
}
function teamApplyParticipantHandoff(){
  const params=new URLSearchParams(location.search);
  if(params.get('from')!=='participants'||params.get('source')!=='daily')return false;
  if(currentMatches.length||teamAssignment||_liveOn||_liveId||_teamStoredLiveId())return false;
  const snapshot=_teamParticipantHandoffSnapshot();
...
function addDirectPlayer(){
  ...
  _directPlayers.push({
    memberId:_rsvpMemberId({name,club:''}),
    name, grade:_dirGrade, level:_newLv, gender:_dirGender, isGuest:_isGuest, ageGroup:_dirAge
  });
  nameEl.value='';
  nameEl.focus();
  renderDirectPlayerList();
...
function _renderRsvpImportSummary(){
  const box=document.getElementById('rsvpImportSummary');
  if(!box)return;
  const s=_lastRsvpImportSummary;
  if(!s){
  ...
  box.innerHTML=`<div class="rsvp-import-summary-title">참가자 불러오기 완료</div>
    ...
    <div class="rsvp-import-summary-next">다음: 참가자 확인 후 청/홍팀 배정</div>`;
}
...
function _rsvpSyncImportedPlayersFromRoster(){
  ...
  try{
    ...
    if(hasBracket){
      ...
      if(competitiveChanged)showWarn(currentSettings?.teamMode
...
/* ═══ TEAM RSVP LINK ═══ */
const RSVP_KEY='kokmatch_team_rsvp_id';
const RSVP_TITLE_KEY='kokmatch_team_rsvp_title';
const RSVP_TITLE_AUTO_KEY='kokmatch_team_rsvp_title_auto';
const RSVP_CLUB_KEY='kokmatch_team_rsvp_club_id';
const RSVP_CREATED_KEY='kokmatch_team_rsvp_created_at';
const RSVP_HISTORY_KEY='kokmatch_team_rsvp_history';
const RSVP_DIRECT_VALUE='__direct__';
const TEAM_SAMPLE_CLUB_ID='sample_mir_club';
let _rsvpId=null;
let _rsvpCreatedAt=null;
...
function isTeamSampleMode(){
  const v=new URLSearchParams(location.search).get('sample');
  return v==='1'||v==='team';
}

function _rsvpShortDate(ts){
  const d=ts?new Date(ts):new Date();
  const mm=String(d.getMonth()+1).padStart(2,'0');
  const dd=String(d.getDate()).padStart(2,'0');
  return `${mm}.${dd}`;
}
function _rsvpCleanLabel(label){
  const s=String(label||'').trim();
  if(!s||s==='클럽 미선택')return '팀전';
...
function _rsvpApplyAutoTitle(force){
  ...
  if(force||_rsvpIsAutoTitle(current,prevAuto)){
    ...
    localStorage.setItem(RSVP_TITLE_AUTO_KEY,title);
    return title;
  }
  return current;
}
function _rsvpTitle(){
  const el=document.getElementById('rsvpTitle');
  return (el&&el.value.trim())||localStorage.getItem(RSVP_TITLE_KEY)||_rsvpDefaultTitle();
}
...
function _rsvpUrl(){
  if(!_rsvpId)return '';
  return _rsvpUrlFor(_rsvpId);
}
function _rsvpUrlFor(id){
  if(!id)return '';
  const base=location.origin+location.pathname.replace(/[^/]*$/,'');
  return base+'rsvp.html?id='+id;
}
function _liveUrl(){
  if(!_liveId)return '';
  const base=location.origin+location.pathname.replace(/[^/]*$/,'');
...
function _rsvpHistoryItem(id){
  if(!id)return null;
  return _rsvpHistoryList().find(x=>x&&x.id===id)||null;
}
function _rsvpActivateEventId(id,{restoreMeta=true,resetMissingMeta=false}={}){
  if(!id)return false;
  const changed=_rsvpId!==id;
  const item=restoreMeta?_rsvpHistoryItem(id):null;
  _rsvpId=id;
  localStorage.setItem(RSVP_KEY,_rsvpId);
  _rsvpCreatedAt=(item&&item.createdAt)||((changed&&resetMissingMeta)?Date.now():(_rsvpCreatedAt||Date.now()));
  localStorage.setItem(RSVP_CREATED_KEY,String(_rsvpCreatedAt));
  const titleEl=document.getElementById('rsvpTitle');
  if(item&&item.title){
    localStorage.setItem(RSVP_TITLE_KEY,item.title);
    if(item.defaultTitle)localStorage.setItem(RSVP_TITLE_AUTO_KEY,item.defaultTitle);
    if(titleEl)titleEl.value=item.title;
  }else if(resetMissingMeta||changed){
    const savedTitle=currentSettings?.rsvpTitle||'';
    const nextTitle=savedTitle||_rsvpDefaultTitle(_rsvpCreatedAt||Date.now());
    localStorage.setItem(RSVP_TITLE_KEY,nextTitle);
    localStorage.setItem(RSVP_TITLE_AUTO_KEY,nextTitle);
    if(titleEl)titleEl.value=nextTitle;
  }
  if(item&&item.source==='directPlayers')localStorage.setItem(RSVP_CLUB_KEY,RSVP_DIRECT_VALUE);
  else if(item&&item.clubId)localStorage.setItem(RSVP_CLUB_KEY,item.clubId);
  else if(currentSettings?.rsvpClubId)localStorage.setItem(RSVP_CLUB_KEY,currentSettings.rsvpClubId);
  return changed;
}
function _rsvpEnsureCurrentEventLink({silent=true,createIfMissing=false}={}){
  if(!currentMatches.length)return false;
  let ownerId=_currentBracketRsvpId();
  if(!ownerId&&createIfMissing){
    ownerId=_rsvpGenId();
    _rsvpCreatedAt=Date.now();
    _teamParticipantSourceRsvpId=ownerId;
    currentSettings=currentSettings||{};
    currentSettings.rsvpId=ownerId;
    currentSettings.rsvpTitle=_rsvpDefaultTitle(_rsvpCreatedAt);
    currentSettings.rsvpClubId=_rsvpSelectedSource();
  }
  if(!ownerId)return false;
  if(_teamParticipantSourceRsvpId!==ownerId)_teamParticipantSourceRsvpId=ownerId;
...
function _rsvpEventPayload(){
  ...
  return {
    matchMode:currentSettings?.teamMode===false?'free':(_teamUsesFixedTeams()?'team':'free'),
    ...
    expiresAt:liveActive&&_liveMatchStartedAt?(_liveMatchStartedAt+LIVE_TTL_MS):null,
...
async function rsvpPushEventState(){
  if(currentMatches.length){
    _rsvpEnsureCurrentEventLink({silent:true});
    if(!_currentBracketRsvpId()||_rsvpId!==_currentBracketRsvpId())return null;
  }
...
function _rsvpRefreshRostersFromStorage(){
  if(_rsvpRosterRefreshLock)return;
  if(typeof loadRosters!=='function')return;
  try{
    _rsvpRosterRefreshLock=true;
    loadRosters();
  }catch(e){
  }finally{
    _rsvpRosterRefreshLock=false;
  }
}
function _rsvpClubs(){
  if(isTeamSampleMode()){
    const sample=_teamSampleRoster();
    const current=(rosters.clubs||[]).filter(c=>(c.members||[]).length&&c.id!==sample.id);
    return [sample,...current];
  }
  _rsvpRefreshRostersFromStorage();
...
function _rsvpSelectedSource(){
  ...
  if(isTeamSampleMode()){
    ...
    if(clubs.some(c=>c.id===TEAM_SAMPLE_CLUB_ID))return TEAM_SAMPLE_CLUB_ID;
    return RSVP_DIRECT_VALUE;
  }
...
async function rsvpSetClub(value){
  ...
  if(value&&(!_rsvpId||!localStorage.getItem(RSVP_TITLE_KEY))){
...
function _rsvpMergedMemberProfile(memberId,fallback={},rosterMap=null){
  const roster=memberId?(rosterMap||_rsvpRosterMemberMap()).get(memberId):null;
  const base=roster||{};
  const gender=base.gender||fallback.gender||'남';
  const fallbackLevel=Number.isFinite(+fallback.level)?+fallback.level:4;
  const grade=base.grade||fallback.grade||levelToGrade(fallbackLevel,gender)||'C';
  return {
    memberId,
    ...
    grade,
    gender,
    ageGroup:base.ageGroup||fallback.ageGroup||'40대',
    isClubOfficial:Object.prototype.hasOwnProperty.call(base,'isClubOfficial')?!!base.isClubOfficial:!!fallback.isClubOfficial
  };
}
...
function rsvpSaveLocal(){
  localStorage.setItem(RSVP_TITLE_KEY,_rsvpTitle());
  localStorage.removeItem('kokmatch_team_rsvp_guest_limit');
  if(currentMatches.length&&_currentBracketRsvpId()){
    currentSettings.rsvpTitle=_rsvpTitle();
    currentSettings.rsvpClubId=_rsvpSelectedSource();
  }
...
function rsvpEnsureId(){
  ...
  if(!_rsvpCreatedAt){
    const saved=parseInt(localStorage.getItem(RSVP_CREATED_KEY)||'0',10);
    _rsvpCreatedAt=Number.isFinite(saved)&&saved>0?saved:Date.now();
    localStorage.setItem(RSVP_CREATED_KEY,String(_rsvpCreatedAt));
  }
  _rsvpApplyAutoTitle(false);
  return _rsvpId;
}
function _rsvpHistoryList(){
  try{
    const list=JSON.parse(localStorage.getItem(RSVP_HISTORY_KEY)||'[]')||[];
...
function _rsvpRememberHistory(){
  ...
  const item={
    ...
    updatedAt:Date.now(),
    url:_rsvpUrl(),
    defaultTitle:_rsvpDefaultTitle(_rsvpCreatedAt||Date.now())
  };
  list=[item,...list.filter(x=>x&&x.id!==_rsvpId)].slice(0,20);
  try{localStorage.setItem(RSVP_HISTORY_KEY,JSON.stringify(list));}catch(e){}
}
function _rsvpSavedDate(ts){
  if(!ts)return '';
  try{
...
function rsvpRenderSavedBox(){
  ...
  if(!box)return;
  const list=_rsvpHistoryList().slice(0,6);
  if(!list.length){box.innerHTML='';return;}
  box.className='rsvp-archive-wrap';
  const rows=list.map(item=>{
    const active=item.id===_rsvpId;
    ...
    return `<div class="rsvp-saved-row">
      ...
      <button class="rsvp-saved-btn danger" onclick="rsvpRemoveSaved('${esc(item.id)}')">삭제</button>
    </div>`;
  }).join('');
...
async function rsvpCopySavedLink(id){
...
function rsvpLoadSaved(id){
  ...
  localStorage.setItem(RSVP_KEY,_rsvpId);
  localStorage.setItem(RSVP_CREATED_KEY,String(_rsvpCreatedAt));
  localStorage.setItem(RSVP_TITLE_KEY,item.title||_rsvpDefaultTitle());
  if(item.defaultTitle)localStorage.setItem(RSVP_TITLE_AUTO_KEY,item.defaultTitle);
  if(item.source==='directPlayers')localStorage.setItem(RSVP_CLUB_KEY,RSVP_DIRECT_VALUE);
  else if(item.clubId)localStorage.setItem(RSVP_CLUB_KEY,item.clubId);
  const titleEl=document.getElementById('rsvpTitle');
  if(titleEl)titleEl.value=item.title||_rsvpDefaultTitle();
  _rsvpResponses={};
...
function rsvpLoad(){
  ...
  const savedCreated=parseInt(localStorage.getItem(RSVP_CREATED_KEY)||'0',10);
...
function rsvpStartListener(){
  if(!_rsvpId||!_fbDb)return;
  const path=_rsvpPath()+'/responses';
  if(_rsvpListening&&_rsvpListenPath===path)return;
  if(_rsvpListening&&_rsvpListenPath){
    try{_fbDb.ref(_rsvpListenPath).off();}catch(e){}
  }
  _rsvpListening=true;
  ...
  const rows=filtered.map(item=>{
    ...
    return `<div class="rsvp-admin-row ${esc(status)}">
      ...
      <details class="rsvp-admin-pair" ${partner.memberId?'open':''}>
        ...
        <div class="rsvp-admin-pair-body">
          <select id="rsvpAdminP_${esc(m.id)}">${_rsvpAdminPartnerOptions(m.id,partner.memberId||'')}</select>
          <button class="rsvp-action-btn primary" onclick="rsvpSetPartnerRequest('${esc(m.id)}')">P 저장</button>
        </div>
      </details>
    </div>`;
  }).join('');
...
function _autoFlowPanel(title,value,note,cls='',target=''){
  const safeTarget=target?esc(target):'';
  const interactive=!!safeTarget;
...
function teamLiveOpenScoreboard(){
  if(currentSettings?.teamMode===false){
    showTab('summary');
...
function teamLiveOpenPanel(target){
  if(currentSettings?.teamMode===false&&(target==='scoreboard'||target==='live'))target='summary';
  const map={
    ...
    summary:{tab:'result',id:'tabSummary'},
...
function renderAutoFlowDashboard(){
  ...
  try{
    ...
    /* 늦음·뒷풀이는 이 화면에서 설정할 수 없게 된 뒤로 늘 0입니다(v1.10.662) —
    ...
    const stepState=(step)=>{
      ...
      const stepIdx=Math.max(...steps.map(s=>order.indexOf(s)).filter(n=>n>=0));
      if(stepIdx>=0&&stepIdx<idx)return 'done';
      return '';
    };
    const actionHtml={
...
function rsvpRender(){
  ...
  const summary=document.getElementById('rsvpSummary');
  if(!box)return;
  if(currentMatches.length)_rsvpEnsureCurrentEventLink({silent:true});
  rsvpRenderClubSelect();
  rsvpRenderSavedBox();
...
async function rsvpSetGuestStatus(memberId,guestIndex,status){
  ...
  if(prev.guest){
    const single=_rsvpNormalizeGuestForTeam(prev.guest,profile.ageGroup||'40대');
    const exists=single&&rawGuests.some(g=>{
      const item=_rsvpNormalizeGuestForTeam(g,profile.ageGroup||'40대');
  ...
  const guests=rawGuests.map(g=>_rsvpNormalizeGuestForTeam(g,profile.ageGroup||'40대')).filter(Boolean);
...
async function rsvpSetPartnerRequest(memberId){
  ...
  const member=rosterMap.get(memberId);
  if(!member)return;
  const select=document.getElementById('rsvpAdminP_'+memberId);
  ...
  _rsvpResponses[memberId]=payload;
  if(partner){
    const pPrev=_rsvpResponses?.[partner.id]||{};
    const partnerProfile=_rsvpMergedMemberProfile(partner.id,pPrev,rosterMap);
    ...
    _rsvpResponses[partner.id]=partnerPayload;
  }else if(prev.partnerRequest&&prev.partnerRequest.memberId&&_rsvpResponses[prev.partnerRequest.memberId]?.partnerRequest?.memberId===memberId){
    const pId=prev.partnerRequest.memberId;
    _rsvpResponses[pId]={..._rsvpResponses[pId],partnerRequest:null,updatedAt:now,source:'admin-rsvp'};
  }
  if(_fbDb){
    await _fbDb.ref(_rsvpPath()+'/responses/'+memberId).set(payload).catch(()=>{});
    if(partner){
      await _fbDb.ref(_rsvpPath()+'/responses/'+partner.id).set(_rsvpResponses[partner.id]).catch(()=>{});
    }else if(prev.partnerRequest&&prev.partnerRequest.memberId&&_rsvpResponses[prev.partnerRequest.memberId]){
...
function _legacyRsvpImportAttendees(){
  ...
  const finalNameByMemberId=new Map();
  ...
  attendees.forEach(r=>{
    ...
    const memberProfile=_rsvpMergedMemberProfile(memberId,r,rosterMap);
    if(_rsvpIsAttending(r.status)){
      memberName=add(memberProfile,'member');
    }
    ...
    _rsvpResponseGuests(r).forEach((g,idx)=>{
      ...
      add({
        ...
        grade:g.grade||'C',
        gender:g.gender||'남',
        ageGroup:g.ageGroup||memberProfile.ageGroup||'40대',
        club:memberProfile.club||r.club,
        isGuest:true
  ...
  memberAttendees.forEach(r=>{
    ...
    const partnerId=req?.memberId||req?.id||'';
    const b=partnerId?finalNameByMemberId.get(partnerId):'';
    if(!a||!b||a===b)return;
    const key=[a,b].sort().join('|');
    if(pairKeys.has(key)||pairUsedNames.has(a)||pairUsedNames.has(b))return;
    pairKeys.add(key);
    pairUsedNames.add(a);
    pairUsedNames.add(b);
    rsvpPairs.push([a,b]);
  ...
  _directPlayers=next;
  _partners=rsvpPairs.map((members,i)=>({
    id:'p_rsvp_'+Date.now()+'_'+i,
    members,
...
function _teamSamplePlayers(){
  const names=[
    ['장세훈','A','남','40대'],['백철민','B','남','40대'],['이은하','A','여','40대'],['최애랑','A','여','40대'],
    ['김재관','C','남','40대'],['이현','C','남','40대'],['정종필','C','남','50대'],['조계홍','C','남','40대'],
    ['김민현','B','남','40대'],['김종길','C','남','50대'],['박대현','B','남','40대'],['조병훈','C','남','40대'],
    ['금민경','A','여','40대'],['김미정','A','여','40대'],['김민선','A','여','40대'],['안세원','B','남','40대'],
    ['이성원','B','남','40대'],['송은정','B','여','40대'],['정인기','B','남','40대'],['현지영','B','여','40대'],
    ['김병주','C','남','40대'],['최경민','C','남','50대'],['윤성호','C','남','40대'],['박지은','C','여','40대'],
    ['이준원','C','남','40대'],['김지아','D','여','30대'],['유재석','D','남','40대'],['문선영','D','여','40대'],
    ['강태오','D','남','30대'],['오하늘','D','여','30대']
  ];
  return names.map(([name,grade,gender,ageGroup])=>({
    name, grade, gender, level:gradeToLevel(grade,gender)||4,
    ageGroup:ageGroup||'40대', isGuest:false
  }));
}
function _teamSampleRoster(){
  return {
    id:TEAM_SAMPLE_CLUB_ID,
    name:'미르클럽',
    members:_teamSamplePlayers().map(({name,grade,gender,ageGroup})=>({name,grade,gender,ageGroup}))
  };
}
function _teamSampleResponses(){
  ...
  _rsvpSessionMembers().forEach((m,i)=>{
    ...
    ]:i===15?[{name:'현장게스트B',gender:'여',grade:'C',ageGroup:'40대'}]:[];
    out[m.id]={
...
function applyTeamSampleData(){
  _fbDb=null;
  _fbReady=false;
  const sampleRoster=_teamSampleRoster();
  rosters={clubs:[sampleRoster],_lvVersion:LV_VERSION};
  localStorage.setItem(RSVP_CLUB_KEY,TEAM_SAMPLE_CLUB_ID);
  _rsvpId='TFAKE201';
  _rsvpCreatedAt=Date.now()-3600000;
  const titleEl=document.getElementById('rsvpTitle');
  if(titleEl)titleEl.value='미르클럽 팀전 06.18';
  _directPlayers=_teamSamplePlayers();
  _teamWanted=true;
  _teamModeOverride=null;
  captains={blue:{leader:'장세훈',sub:'김민현'},white:{leader:'금민경',sub:'이성원'}};
  teamAssignment=null;
  currentMatches=[];
  currentParticipants=[];
  currentSettings={};
  ...
  setTimeout(()=>{
    ...
    renderAutoFlowDashboard();
    showTab('bracket');
    const badge=document.getElementById('autoFlowBadge');
    if(badge)badge.textContent='샘플';
    setSaveStatus('','샘플 모드 · 저장 안 함');
  },280);
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.mob-hide').forEach(el => el.classList.remove('mob-hide'));
  // 하단 탭 초기 활성화
  const btn = document.getElementById('bnav-dashboard');
  if(btn) btn.classList.add('active');
});
