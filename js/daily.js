...
// 파일 뒤편에 있을 때는 이 파일을 잘라 쓰는 테스트에서 정의가 빠져 터졌다(2026-09-03).
...
let _dailyPreparedOperation='';
try{_dailyPreparedOperation=sessionStorage.getItem('kokmatch_daily_prepared_operation_v1')==='daily'?'daily':'';}catch(e){}
...
let _dailyServerSyncPromise=null;
...
function _dailyMarkOperationStarted(operationAt){
  const at=Number(operationAt)||_dailyNow();
  _dailyOperationStarted=true;
  if(!_dailyOperationStartedAt)_dailyOperationStartedAt=at;
  return _dailyOperationStartedAt;
}
function _dailyMarkLiveAddition(player,options){
  if(!player||!_dailyOperationStarted)return false;
  const at=Number(options?.at)||_dailyNow();
  const startedAt=_dailyOperationStartedAt||_dailyMarkOperationStarted(at);
  if(at<startedAt)return false;
  player.registrationCancelled=false;
  player.liveAddedAt=at;
  player.liveAddedBy=options?.actorId||'system-admin';
  player.liveAddedByName=options?.actorName||'관리자';
  player.liveAddedSource=options?.source||'system-admin-live-add';
  player.liveAddedOrigin=options?.origin||'existing';
  player.liveAddedCandidateKey=options?.candidateKey||`player:${player.id}`;
  player.liveAddedOperationId=options?.operationId||`local-add-${player.id}-${at}`;
  return true;
}
function _dailyClearLiveAddition(player){
  if(!player)return;
  player.liveAddedAt=0;
  player.liveAddedBy='';
  player.liveAddedByName='';
  player.liveAddedSource='';
  player.liveAddedOrigin='';
  player.liveAddedCandidateKey='';
  player.liveAddedOperationId='';
}
...
function _dailyNormalize(raw){
  ...
  const now=_dailyNow();
  const status=_dailyNormalizeStatus(raw.status||'wait');
  const games=Math.max(0,Number(raw.games||0));
  const fairExpected=Object.prototype.hasOwnProperty.call(raw,'fairExpected')&&Number.isFinite(Number(raw.fairExpected))
    ?Math.max(0,Number(raw.fairExpected))
    :games+((status==='playing'||raw.currentMatchId)?1:0);
  return {
    id:raw.id||_dailyId(),
    ...
    status,
    preArrivalVisible:raw.preArrivalVisible===true,
    registrationCancelled:raw.registrationCancelled===true,
    ...
    temporaryOfficialGrantedAt:Number(raw.temporaryOfficialGrantedAt||0),
    temporaryOfficialGrantedBy:raw.temporaryOfficialGrantedBy||'',
    temporaryOfficialGrantedByName:raw.temporaryOfficialGrantedByName||'',
    ...
    liveAddedAt:Number(raw.liveAddedAt||0),
    liveAddedBy:raw.liveAddedBy||'',
    liveAddedByName:raw.liveAddedByName||'',
    liveAddedSource:raw.liveAddedSource||'',
    liveAddedOrigin:raw.liveAddedOrigin||'',
    liveAddedCandidateKey:raw.liveAddedCandidateKey||'',
    liveAddedOperationId:raw.liveAddedOperationId||''
  };
}
function _dailyClearTemporaryOfficials(){
  let changed=false;
  _dailyPlayers.forEach(player=>{
    if(!player?.isTemporaryOfficial)return;
    player.isTemporaryOfficial=false;
    player.temporaryOfficialGrantedAt=0;
    player.temporaryOfficialGrantedBy='';
    player.temporaryOfficialGrantedByName='';
    changed=true;
  });
  return changed;
}
...
// 관리자 파트너 지정도 임원 게임신청과 같은 예약 한 곳에 모입니다.
// 선수에게 partnerName 을 박던 갈래는 없앴습니다.
function _dailyReservationOfPlayer(playerId){
  if(!playerId)return null;
  return _dailyReservations.find(r=>_dailyReservationIds(r).includes(playerId))||null;
}
function _dailyPartnerNameOf(playerId){
  const r=_dailyReservationOfPlayer(playerId);
  if(!r)return '';
  const side=(r.team1||[]).includes(playerId)?(r.team1||[]):(r.team2||[]);
  const mateId=side.find(id=>id&&id!==playerId);
  const mate=mateId?_dailyPlayer(mateId):null;
  return mate?_dailyNameText(mate):'';
}
...
function _dailyReservationStatus(r){
  ...
  if(r.mode==='pair'){
    ...
    for(let i=0;i<pool.length-1&&!sameShapeReady;i++)for(let j=i+1;j<pool.length;j++){
      if(_dailySameOpponentShape(ids,[pool[i],pool[j]])){
        sameShapeReady=true;
        break;
      }
    }
...
async function _dailySendQueueYield(idx,q,targetIndex){
  const target=Number(targetIndex)||idx+2;
  const free=target!==idx+2;
  return _dailySendAdminCommand({
    type:'official-queue-yield',
    queueId:q.id,
    token:'dqy_'+_dailyNow().toString(36)+'_'+Math.random().toString(36).slice(2,7),
    expectedQueueIndex:idx+1,
    targetQueueIndex:target,
    // 관리자 드래그는 임의 위치로 옮깁니다. 임원의 '이번만 뒤로'는 한 칸만입니다.
    ...(free?{allowFreeMove:true}:{}),
    expectedPlayerIds:[..._dailyQueueIds(q)],
    expectedTeam1Ids:[...(q.team1||[])],
    expectedTeam2Ids:[...(q.team2||[])],
    source:'system-admin-queue-yield'
  },{action:'대진 순서 변경',tag:'queue-yield'});
}
async function dailyMoveQueueTo(sourceId,targetId){
  ...
  if(_dailyCheckinId){
    const sent=await _dailySendQueueYield(from,_dailyQueue[from],to+1);
    return sent.ok;
  }
...
async function dailyDeleteQueueItem(queueId){
  ...
  if(_dailyCheckinId){
    const sent=await _dailySendAdminCommand({
      ...
      queueId,
      expectedPlayerIds:[..._dailyQueueIds(_dailyQueue[idx])],
      source:'system-admin-queue-delete'
    },{action:'대기 경기 삭제',tag:'queue-delete'});
    return sent.ok;
  }
...
function _dailyNormalizeCourtOrder(order,count){
  ...
  (Array.isArray(order)?order:[]).forEach(v=>{
    ...
    if(c>=1&&c<=12&&!out.includes(c)&&out.length<n)out.push(c);
  });
  for(let c=1;c<=12&&out.length<n;c++)if(!out.includes(c))out.push(c);
  return out.sort((a,b)=>a-b);
}
function _dailyResizeCourtOrder(nextCount,currentCount){
  const current=Math.max(1,Math.min(12,parseInt(currentCount)||_dailyCourtCount()));
  const next=Math.max(1,Math.min(12,parseInt(nextCount)||current));
  const order=_dailyNormalizeCourtOrder(_dailyCourtOrder,current);
  const active=_dailyActiveMatches();
  const activeCourts=new Set(active.map(m=>parseInt(m.court,10)).filter(Boolean));
  while(order.length>next){
    const empty=order.filter(court=>!activeCourts.has(court)).sort((a,b)=>b-a)[0];
    const newest=active.filter(m=>order.includes(parseInt(m.court,10))).sort((a,b)=>
      Math.max(Number(b.autoHandoffAt||0),Number(b.startedAt||0))-
      Math.max(Number(a.autoHandoffAt||0),Number(a.startedAt||0))||
      Number(b.seq||0)-Number(a.seq||0)||Number(b.court||0)-Number(a.court||0)
    )[0];
    const court=empty||parseInt(newest?.court,10);
    if(!court)break;
    order.splice(order.indexOf(court),1);
  }
  for(let court=1;order.length<next&&court<=12;court++)if(!order.includes(court))order.push(court);
  _dailyCourtOrder=order.sort((a,b)=>a-b);
  return _dailyCourtOrder.slice();
}
...
function _dailyCourtAvailable(court,limit){
  ...
  if(!court||!_dailyCourtOrderForUse(cap).includes(court))return false;
...
function _dailyFairExpected(p){
  if(!p)return 0;
  return Number.isFinite(Number(p.fairExpected))?Math.max(0,Number(p.fairExpected)):_dailyFairActual(p);
}
function _dailyFairGap(p){
  return _dailyFairExpected(p)-_dailyFairActual(p);
}
function _dailyFairPriorityBonus(p){
  const gap=Math.max(0,_dailyFairGap(p));
  if(gap<DAILY_FAIR_PRIORITY_GAP)return 0;
  return Math.min(2800,Math.round((gap-0.5)*700));
}
function _dailyFairSummary(){
  const players=_dailyPlayers.filter(p=>['wait','playing'].includes(_dailyNormalizeStatus(p.status)));
  const priority=players.filter(p=>_dailyFairGap(p)>=DAILY_FAIR_PRIORITY_GAP);
  return {players,priority,count:priority.length,normal:priority.length===0};
}
function _dailyApplyFairOpportunity(match){
  if(!match)return;
  const pool=_dailyEligible();
  if(!pool.length)return;
  const slots=Math.max(1,[...(match.team1||[]),...(match.team2||[])].length||4);
  const amount=slots/pool.length;
  const deltas=[];
  pool.forEach(p=>{
    p.fairExpected=_dailyFairExpected(p)+amount;
    deltas.push({id:p.id,amount});
  });
  match.fairExpectedDeltas=deltas;
}
function _dailyRollbackFairOpportunity(match){
  if(!match||!Array.isArray(match.fairExpectedDeltas)||match.fairExpectedRolledBackAt)return;
  match.fairExpectedDeltas.forEach(row=>{
    const p=_dailyPlayer(row?.id);
    if(!p)return;
    p.fairExpected=Math.max(0,_dailyFairExpected(p)-Math.max(0,Number(row?.amount||0)));
  });
  match.fairExpectedRolledBackAt=_dailyNow();
}
...
function dailySave(options){
  try{
    ...
    localStorage.setItem(DAILY_KEY,JSON.stringify({
      ...
      clientStateRevision:_dailyClientStateRevision,
...
function _dailyLoadAsNewDay(s){
  ...
  _dailyServerRevision=0;_dailyServerLastRequestId='';_dailyOfficialInviteToken='';_dailyOfficialInviteHash='';_dailyCapabilityPromise=null;_dailyServerReconcileError='';
...
function dailyLoad(){
  try{
    ...
    const sameCheckinIdentity=!!storedCheckinId&&(!savedCheckinId||storedCheckinId===savedCheckinId);
    _dailyCheckinId=storedCheckinId||savedCheckinId||null;
    _dailyCheckinCreatedAt=storedCheckinCreatedAt||(_dailyCheckinId===savedCheckinId?(s.checkinCreatedAt||s.savedAt||0):0);
    _dailyServerRevision=sameCheckinIdentity?Math.max(0,Number(s.serverRevision||0)):0;
    _dailyServerLastRequestId=sameCheckinIdentity?String(s.serverLastRequestId||''):'';
    _dailyOfficialInviteToken=_dailyCheckinId?String(s.officialInviteToken||''):'';
    _dailyOfficialInviteHash=_dailyCheckinId?String(s.officialInviteHash||''):'';
    _dailyClientStateRevision=Math.max(0,Number(s.clientStateRevision||0));
    _dailyCheckinNeedsPublish=!!(_dailyCheckinId&&sameCheckinIdentity&&s.checkinNeedsPublish);
    _dailyCheckinPublishQueued=_dailyCheckinNeedsPublish;
...
function dailyApplyReviewSample(){
  ...
  _dailyWaveStarts=3;
  _dailyAutoAssign=false;
  _dailyMarkOperationStarted(now-50*60*1000);
...
async function dailyStepCourts(delta){
  ...
  if(!el)return;
  const current=parseInt(el.value)||3;
  const next=Math.max(1,Math.min(12,current+delta));
  if(next===current)return;
  // 게시 후에도 코트 수는 바꿀 수 있게 두되(운영자 판단 ③) 기록은 서버로 모읍니다.
  if(_dailyCheckinId){
    const sent=await _dailySendAdminCommand({
      type:'official-settings-update',
      courts:next,
      expectedCourts:current,
      source:'system-admin-settings'
    },{action:'코트 수 변경',tag:'settings'});
    return sent.ok;
  }
  el.value=next;
  _dailyResizeCourtOrder(next,current);
  _dailySyncControls(next);
  dailySave();
  dailyRender();
  dailyMaybeAutoAssign();
}
...
function _dailyParticipantHandoffSnapshot(){
  const bridge=_dailyRosterBridge();
  return bridge&&typeof bridge.loadHandoff==='function'?bridge.loadHandoff():{players:[],source:'',savedAt:0};
}
function _dailyApplyParticipantProfiles(source){
  const now=_dailyNow();
...
function dailyImportTeamRoster(options={}){
  ...
  const snapshot=options.snapshot||_dailyTeamRosterSnapshot();
  const source=snapshot.players||[];
  if(!source.length){
    alert('가져올 팀전 선수 명단이 없습니다.');
    return;
  }
  if(!options.silent&&_dailyPlayers.length&&!confirm(`현재 민턴LIVE 선수 ${_dailyPlayers.length}명을 지우고\n팀전 선수 ${source.length}명을 가져올까요?`))return;
  _dailyApplyParticipantProfiles(source);
  const setup=document.getElementById('dailySetupDetails');
  if(setup)setup.open=true;
  if(!options.silent)alert(`팀전 선수 ${_dailyPlayers.length}명을 민턴LIVE 명단으로 가져왔습니다.`);
  return true;
}
function dailyApplyParticipantHandoff(){
  const params=new URLSearchParams(location.search);
  if(params.get('from')!=='participants'||params.get('source')!=='team')return false;
  if(_dailyCheckinId||_dailyOperationStarted||_dailyMatches.length)return false;
...
function renderParticipantPreparation(){
  ...
  if(!count&&_dailyPreparedOperation){
    _dailyPreparedOperation='';
    try{sessionStorage.removeItem('kokmatch_daily_prepared_operation_v1');}catch(e){}
  }
  const card=document.getElementById('operationModeCard');
  if(card)card.hidden=!!(_dailyPreparedOperation==='daily'||_dailyOperationStarted);
  ...
  if(hint)hint.textContent=count
    ? `${count}명 준비 완료 · 선택한 운영 방식으로 같은 명단이 이어집니다.`
    : '참가자를 먼저 등록하면 같은 명단으로 원하는 방식을 시작합니다.';
}
function dailyChooseOperation(mode){
  if(mode!=='daily'&&mode!=='team')return;
  if(!_dailyPlayers.length){
    dailyImportRoster();
    return;
  }
  const bridge=_dailyRosterBridge();
  if(bridge&&typeof bridge.handoff==='function')bridge.handoff('daily',_dailyPlayers);
  if(mode==='team'){
    const progressed=_dailyCheckinId||_dailyOperationStarted||_dailyMatches.length;
    const from=progressed?'daily':'participants&source=daily';
    location.href=`team.html?v=${encodeURIComponent(APP_VERSION)}&from=${from}`;
    return;
  }
  _dailyPreparedOperation='daily';
  try{sessionStorage.setItem('kokmatch_daily_prepared_operation_v1','daily');}catch(e){}
  dailyRender();
  const setup=document.getElementById('dailySetupDetails');
  if(setup)setup.open=true;
  const hint=document.getElementById('participantPrepHint');
  if(hint)hint.textContent='민턴LIVE 준비 완료 · 코트 수를 확인하고 대진을 게시하세요.';
  dailyOpenBoardTarget('setup');
}
...
// 참가자 모달의 게스트 추가. 등록 뒤 아직 안 온 선수만 선수 목록에서 도착 전으로 바꿉니다.
async function dailyImportAddGuest(){
...
function dailyImportRoster(){
  ...
  renderDailyImportMembers();
  // 게스트 폼은 접어서 연다 — 명부가 비어 게스트가 유일한 길일 때만 펼친다
...
function _dailyApplyPlayerStatus(p,status,operationAt){
  ...
  if(newlyArrived){
    ...
    p.arrivalRequestKey='';
    _dailyMarkLiveAddition(p,{at:now,origin:'existing',source:'system-admin-arrival'});
  }
  p.status=status;
  if(status==='planned'||status==='invited'){
...
async function dailySetStatus(id,status){
  ...
  if(_dailyCheckinId
    &&['wait','rest','done'].includes(nextStatus)
    &&!['invited','planned'].includes(_dailyNormalizeStatus(p.status))){
    ...
    const sent=await _dailySendAdminCommand({
      ...
      playerName:p.name,
      status:nextStatus,
      expectedStatus:playing?'playing':(p.status||''),
      expectedCurrentMatchId:p.currentMatchId||'',
      expectedLastStatusAt:p.lastStatusAt||0,
      source:'system-admin-player-status'
    },{action:'선수 상태 변경',tag:'status'});
    if(sent.live)return sent.ok;
  }
...
async function dailyStartPair(id){
  if(_dailyBlockPaused({action:'파트너를 지정'}))return;
  const p=_dailyPlayer(id);
  if(!p||p.status==='playing')return;
  if(_dailyReservationOfPlayer(id)){await dailyClearPair(id);return;}
...
async function dailyConfirmPair(id){
  ...
  // 관리자 지정도 임원 접수와 같은 게임신청 한 곳으로 들어갑니다.
  const clash=[a,b].filter(player=>_dailyReservationOfPlayer(player.id));
  if(clash.length){
    alert(`${clash.map(player=>player.name).join(', ')} 선수는 이미 게임신청에 포함되어 있습니다.\n먼저 그 신청을 정리한 뒤 묶어 주세요.`);
    dailyCancelPair();return;
  }
  if(_dailyCheckinId){
    _dailyPairSelectId=null;
    const sent=await _dailySendAdminCommand({
      ...
      source:'system-admin-partner'
    },{action:'파트너 지정',tag:'partner'});
    dailyRender();
    return sent.ok;
  }
  ...
  _dailyReleaseTemporaryQueueForReservationIds([a.id,b.id]);
  _dailyPairSelectId=null;
  _dailyNext=null;
  dailySave();dailyRender();dailyRecommend();
}
async function dailyClearPair(id){
  if(_dailyBlockServerSync({action:'파트너 지정 해제'}))return;
  if(_dailyBlockPaused({action:'파트너 지정을 해제'}))return;
  const p=_dailyPlayer(id);
  if(!p)return;
  const r=_dailyReservationOfPlayer(id);
  if(_dailyCheckinId){
    _dailyPairSelectId=null;
    if(!r){dailyRender();return false;}
    const sent=await _dailySendAdminCommand({
  ...
  if(r){
    _dailyReservations=_dailyReservations.filter(x=>x.id!==r.id);
...
async function dailyDeleteReservation(id){
...
async function dailyPromoteReservation(id){
  if(_dailyBlockServerSync({action:'게임신청 반영'}))return;
  if(_dailyBlockPaused({action:'게임신청을 대진에 반영'}))return;
  if(_dailyCheckinId){
    const sent=await _dailySendAdminCommand({
      type:'official-reservation-promote',
      reservationId:id,
      source:'system-admin-reservation-promote'
    },{action:'게임신청 반영',tag:'reservation-promote'});
    return sent.ok;
  }
...
function _dailyUpdatePlayerToolState(){
  ...
  ['all','current','wait','playing','rest','done','planned','queued'].forEach(mode=>{
...
function _dailyPlayerMatchesFilter(p){
  ...
  if(_dailyPlayerFilter==='current')return ['wait','playing','rest'].includes(_dailyNormalizeStatus(p.status));
  if(_dailyPlayerFilter==='planned')return ['invited','planned'].includes(_dailyNormalizeStatus(p.status));
...
async function dailyRenamePlayer(id){
  if(!_dailyCanChangeRoster())return;
  const p=_dailyPlayer(id);
  if(!p)return;
  const name=prompt(p.isGuest?'게스트 이름 수정':'이름 변경',p.name);
  if(name==null)return;
  ...
  if(_dailyCheckinId){
    const sent=await _dailySendAdminCommand({
      type:'official-player-rename',
      playerId:p.id,
      name:next,
      expectedName:p.name,
      source:'system-admin-player-rename'
    },{action:'이름 변경',tag:'player-rename'});
    return sent.ok;
  }
  const prev=p.name;
  p.name=next;
...
function _dailyGuestRenameButton(p){
  if(!p?.isGuest)return '';
  const disabled=_dailyPaused?'disabled':'';
  return `<button type="button" class="daily-guest-rename" ${disabled} onclick="dailyRenamePlayer('${p.id}')" aria-label="${esc(p.name)} 게스트 이름 수정" title="게스트 이름 수정">${_dailyIcon('edit')}</button>`;
}
function _dailyPlayerRemoveButton(p){
  const playing=_dailyNormalizeStatus(p?.status)==='playing'||!!p?.currentMatchId;
  const disabled=_dailyPaused||playing?'disabled':'';
  const title=playing?'경기 정리 후 삭제':'선수 삭제';
  return `<button type="button" class="daily-player-remove-inline" ${disabled} onclick="dailyRemovePlayer('${p.id}')" aria-label="${esc(p.name)} 선수 삭제" title="${title}">${_dailyIcon('trash')}</button>`;
}
...
function _dailyPlayerRowActions(p){
  ...
  if(_dailyPlayerTool==='pair'){
    ...
    const partnerName=_dailyPartnerNameOf(p.id);
    if(partnerName)return `<span class="daily-player-actions"><button type="button" class="pair" ${dis} onclick="dailyClearPair('${p.id}')">해제 · ${esc(partnerName)}</button></span>`;
  ...
  const buttons=actions.map(b=>`<button type="button" class="${b.cls}" ${dis} onclick="dailySetStatus('${p.id}','${b.status}')">${b.label}</button>`).join('');
...
function _dailyIcon(name){
  const paths={
    ...
    edit:'M4.5 19.5l4.2-.8L19 8.4a2.1 2.1 0 0 0-3-3L5.3 16.1l-.8 3.4zM14.8 6.6l3 3'
  };
  const d=paths[name];
...
async function dailyRemovePlayer(id){
  ...
  if(_dailyCheckinId){
    const sent=await _dailySendAdminCommand({
      type:'official-player-remove',
      playerId:p.id,
      playerName:p.name,
      expectedName:p.name,
      source:'system-admin-player-remove'
    },{action:'선수 제외',tag:'player-remove'});
    return sent.ok;
  }
...
function dailyReset(){
  ...
  _dailyTeamLocked=false;
  _dailyVoteDeadlineAt='';
  if(_dailyCheckinOwnershipVerified&&_dailyCheckinId){
    _dailyRemoveOwnedCheckin(_dailyCheckinPath(),_dailyIdentitySnapshot(),false);
  }
  _dailyClearAdminGrant();
...
async function dailyToggleFinishMode(){
  ...
  if(!_dailyFinishMode){
    ...
    if(_dailyCheckinId){
      const sent=await _dailySendAdminCommand({
        type:'official-finish-mode',
        finishMode:true,
        source:'system-admin-finish-mode'
      },{action:'마무리 전환',tag:'finish'});
      return sent.ok;
    }
...
function _dailyFlexibleMatch(four){
  ...
  combos.forEach(c=>{
    ...
    score+=_dailyPartnerRepeatPenalty(t1[0].partnerCount[t1[1].name]||0);
    score+=_dailyPartnerRepeatPenalty(t2[0].partnerCount[t2[1].name]||0);
    if(score<bestScore){
      bestScore=score;
      best=match;
    }
...
function _dailyScoreMatch(m,strict){
  ...
  teams.forEach(t=>{score+=_dailyPartnerRepeatPenalty(t[0].partnerCount[t[1].name]||0);});
...
function _dailyTailQueueReplacementCandidates(q,currentId,idx){
  ...
  for(let sourceIdx=_dailyQueue.length-1;sourceIdx>idx;sourceIdx--){
    ...
    if(!sourceQ||sourceQ.reservationId||_dailyQueueRestPassActive(sourceQ))continue;
...
async function dailyPickActiveReplacement(matchId,side,pos){
  ...
  if(_dailyCheckinId){
    const sent=await _dailySendAdminCommand({
      ...
      expectedStartedAt:m.startedAt||0,
      expectedPlayerIds:[...m.team1,...m.team2],
      source:'system-admin-active-replace'
    },{action:'진행 선수 교체',tag:'active-replace'});
    return sent.ok;
  }
  _dailyApplyActiveReplaceLocal(m,currentId,candidate.id);
  dailyEnsureQueue();
  dailySave();
  dailyRender();
  dailyMaybeAutoAssign();
}
// 서버 재생과 로컬(게시 전) 경로가 같은 함수를 씁니다.
// 대기 선수 투입이면 나간 선수는 휴식, 맞교환이면 두 경기의 명단만 서로 바꿉니다.
function _dailyApplyActiveReplaceLocal(m,outId,inId,operationAt){
  const at=Number(operationAt)||_dailyNow();
  ...
  if(inPlayer){
    _dailyCancelReservationsForPlayer(inPlayer.id,'진행 중 경기 선수교체로 게임신청이 자동 취소됐습니다.','admin-active-replace');
    _dailyQueue=_dailyQueue.filter(q=>!_dailyQueueIds(q).includes(inPlayer.id));
    inPlayer.status='playing';
    inPlayer.currentMatchId=m.id;
    inPlayer.afterMatchStatus=null;
...
function _dailyManualActiveDefaultCourt(){
  ...
  for(const c of _dailyCourtOrderForUse()){
...
function dailySetManualActiveCourt(court){
  ...
  if(!_dailyCourtOrderForUse().includes(court))return;
...
function dailyRenderManualActiveModal(){
  ...
  const courtIds=_dailyCourtOrderForUse();
  ...
  if(courtGrid){
    courtGrid.innerHTML=courtIds.map(c=>{
  ...
  if(hint)hint.textContent=transition
    ? `등록 ${registeredCount}/${courtIds.length}`
...
async function dailyConfirmManualActiveMatch(){
  if(_dailyBlockServerSync({action:'수동 경기 등록'}))return;
  if(_dailyBlockPaused({action:'진행 경기를 등록'}))return;
  const mode=_dailyManualActiveMode();
  const transition=mode==='transition';
  const court=parseInt(_dailyManualActiveDraft.court,10)||0;
  if(!Number.isFinite(court)||!_dailyCourtOrderForUse().includes(court)){
  ...
  if(_dailyCheckinId){
    const operationId=_dailyAdminOperationId('manual-match');
    const sent=await _dailySendAdminCommand({
      type:'official-manual-match',
      operationId,
      matchId:`sm_${operationId}`,
      court,
      team1Ids:ids.slice(0,2),
      team2Ids:ids.slice(2,4),
      transition,
      reservationLabel,
      source:'system-admin-manual-match'
    },{action:'경기 등록',tag:'manual-match'});
    if(sent.ok){
      if(transition){
        _dailyManualActiveDraft={mode:'transition',court:_dailyManualActiveDefaultCourt(),ids:[],registeredCount:(_dailyManualActiveDraft.registeredCount||0)+1};
        dailyRenderManualActiveModal();
      }else{
        closeDailyManualActiveModal();
      }
...
async function dailyRegenerateQueueItem(queueId){
  ...
  if(_dailyCheckinId){
    const sent=await _dailySendAdminCommand({
      ...
      queueId,
      expectedPlayerIds:[..._dailyQueueIds(_dailyQueue[idx])],
      source:'system-admin-queue-regenerate'
    },{action:'대기 경기 재생성',tag:'queue-regen'});
    return sent.ok;
  }
  const used=new Set();
...
function dailyStartQueueItem(queueId,options){
  ...
  _dailyQueue.splice(idx,1);
  _dailyMarkOperationStarted(options.startedAt||options.operationAt);
  ...
  _dailyMatches.push(startedMatch);
  _dailyApplyFairOpportunity(startedMatch);
  ids.forEach(pid=>{
    const p=_dailyPlayer(pid);
    p.afterMatchStatus=null;
    p.status='playing';
...
function _dailyRenderQueueItem(q,idx,mode){
  ...
  return `<div class="daily-queue-item ${expected?'expected':reserved?'locked':'flex'}" data-daily-queue-id="${q.id}" data-daily-queue-idx="${idx}" ${dragAttrs}>
    <div class="daily-queue-head">
      ...
      ${canStart?`<div class="daily-queue-actions single"><button class="daily-mini-btn primary-action" onclick="dailyOperatorStartQueueItem('${q.id}')">${teamMode?'배정':'시작'}</button></div>`:''}
    </div>
    ${boardHtml}
    ${urgent?'':editHtml}
...
function _dailyResultQualityChips(st){
  const chips=[];
  const fair=_dailyFairSummary();
  chips.push({label:`완료 ${st.completed.length}경기`,cls:'ok'});
  chips.push({label:fair.count?`공정 보정 ${fair.count}명`:'공정 배정 정상',cls:fair.count?'warn':'ok'});
...
function _dailyPublicEvent(){
  ...
  const queuePayload=(q,idx,extra)=>{
    ...
    return {
      ...
      restPass:restPass&&!extra?JSON.parse(JSON.stringify(q.restPass)):null,
  ...
  const operatingCourtIds=_dailyCourtOrderForUse();
  const operatingCourtSet=new Set(operatingCourtIds);
  const drainingCourtIds=[...new Set(st.active.map(match=>Number(match.court||0))
    .filter(court=>court&&!operatingCourtSet.has(court)))].sort((a,b)=>a-b);
...
function _dailyCompleteUndoGuard(){
  return JSON.stringify({
    players:_dailyPlayers.map(p=>[p.id,p.status,p.currentMatchId||'',p.afterMatchStatus||'',p.games||0,Number(p.fairExpected||0),p.lastStatusAt||0]),
...
async function dailyCancelMatch(id){
  ...
  if(_dailyCheckinId){
    const sent=await _dailySendAdminCommand({
      type:'official-court-cancel',
      matchId:m.id,
      court:m.court,
      expectedStartedAt:m.startedAt||0,
      expectedPlayerIds:[...m.team1,...m.team2],
      source:'system-admin-court-cancel'
    },{action:'경기 취소',tag:'court-cancel'});
    return sent.ok;
  }
  _dailyApplyMatchCancel(m);
  dailySave();dailyRender();
  dailyMaybeAutoAssign();
}
// 취소는 완료와 반대로 games·partnerCount 를 건드리지 않고 경기 전 상태로 되돌립니다.
// 서버 재생도 이 함수를 씁니다 — 관리자 원본에는 previousStatuses 가 있어
// 서버보다 정확히 복원할 수 있습니다.
function _dailyApplyMatchCancel(m,operationAt){
  if(!m||m.completedAt||m.cancelledAt)return false;
  const at=Number(operationAt)||_dailyNow();
  _dailyRollbackFairOpportunity(m);
  [...m.team1,...m.team2].forEach(pid=>{
    ...
    p.deferReason='';
    p.currentMatchId=null;
    if(p.status==='wait')p.waitFrom=at;
    p.lastStatusAt=at;
    p.restPausedMs=0;
  });
  m.cancelledAt=at;
...
function dailyRenderStartGuide(){
  ...
  if(!el)return;
  if(!_dailyPreparedOperation&&!_dailyOperationStarted){
    el.hidden=true;
    el.innerHTML='';
    return;
  }
...
function _dailyHeadcountInfo(){
  const counts={invited:0,planned:0,wait:0,playing:0,rest:0,done:0};
  _dailyPlayers.forEach(player=>{
    if(player.registrationCancelled)return;
    const status=_dailyNormalizeStatus(player.status);
    counts[status]=(counts[status]||0)+1;
  });
  const preArrival=counts.invited+counts.planned;
  const current=counts.wait+counts.playing+counts.rest;
  return {...counts,preArrival,current,total:_dailyPlayers.length};
}
function dailyOpenPlayerStatus(filter){
  setDailyPlayerFilter(filter);
...
function dailyRenderHeadcount(){
  const el=document.getElementById('dailyHeadcount');
  if(!el)return;
  const count=_dailyHeadcountInfo();
  const cards=[
    {label:'전체',value:count.total,filter:'all',cls:'primary'},
    {label:'현장',value:count.current,filter:'current',cls:'primary'},
    {label:'경기중',value:count.playing,filter:'playing'},
    {label:'대기',value:count.wait,filter:'wait'},
    {label:'휴식',value:count.rest,filter:'rest'},
...
function _dailyLiveAdditionRows(){
  ...
  if(!startedAt)return [];
  return _dailyPlayers
    .filter(player=>Number(player.liveAddedAt||0)>=startedAt)
    .sort((a,b)=>Number(b.liveAddedAt||0)-Number(a.liveAddedAt||0));
}
function _dailyCanCancelLiveAddition(player){
  return !!(
    player
    &&!player.registrationCancelled
    &&Number(player.liveAddedAt||0)>0
    &&Number(player.games||0)===0
    &&player.status!=='playing'
    &&!player.currentMatchId
  );
}
function _dailyLiveAdditionTime(value){
  const at=Number(value||0);
  return at?new Date(at).toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit'}):'';
}
function dailyRenderLiveAdditions(){
  const el=document.getElementById('dailyLiveAdditions');
  if(!el)return;
  const rows=_dailyLiveAdditionRows();
  el.hidden=!rows.length;
  if(!rows.length){el.innerHTML='';return;}
  el.innerHTML=`<div class="daily-live-additions-head"><b>라이브 후 추가</b><span>${rows.length}명</span></div>${rows.map(player=>{
    const cancellable=_dailyCanCancelLiveAddition(player);
    const busy=_dailyLiveAdditionBusy===player.id;
    const detail=[_dailyLiveAdditionTime(player.liveAddedAt),player.liveAddedByName||'관리자',_dailyCheckinStatusLabel(player.status)].filter(Boolean).join(' · ');
...
function dailyRenderOpsStats(){
  ...
  const operatingCourts=new Set(_dailyCourtOrderForUse());
  const drainingCourts=[...new Set(activeMatches
    .map(m=>parseInt(m.court,10)||0)
    .filter(court=>court&&!operatingCourts.has(court)))].sort((a,b)=>a-b);
...
function _dailyAdminOperationId(prefix){
  const tag=String(prefix||'op').replace(/[^a-zA-Z0-9_-]/g,'').slice(0,24)||'op';
  try{
    const bytes=new Uint8Array(12);
    globalThis.crypto.getRandomValues(bytes);
    return `dov2_${tag}_${Array.from(bytes,b=>b.toString(16).padStart(2,'0')).join('')}`;
  }catch(e){
    return `dov2_${tag}_${_dailyNow().toString(36)}_${Math.random().toString(36).slice(2,12)}`;
  }
}
function _dailyTemporaryOfficialOperationId(enabled){
  return _dailyAdminOperationId(enabled?'temp-grant':'temp-revoke');
}
// 관리자 동작 하나를 서버 명령으로 보냅니다. 세션이 게시된 뒤에는 관리자도
// 임원과 같은 경로로만 세션을 고칩니다 — 통째로 덮어쓰는 쪽과 부분 수정하는 쪽이
// 한 문서에 같이 붙어 있던 것이 2026-08-02 밤 사고의 원인이었습니다.
//
// 아직 게시 전이면 {live:false} 를 돌려주고, 그때만 호출부가 예전처럼 직접 씁니다.
...
async function _dailySendAdminCommand(command,options){
  ...
  try{
    if(!await _dailyEnsureAdminGrant())throw new Error('관리자 운영 연결을 확인하지 못했습니다.');
    // 자동 배정으로 시작된 경기처럼 아직 로컬에만 있는 것이 있으면, 서버는 그 경기를
    // 모르는 채로 명령을 받아 "찾지 못했습니다"로 거절합니다. 명령을 보내기 전에
    // 관리자 원본을 먼저 올려 서버가 같은 것을 보게 합니다(2026-08-04 실측).
    //
    // _dailyCheckinNeedsPublish 로 가르지 않습니다. 이 플래그는 무관한 다른 게시가
    // 끝나도 꺼져서, 정작 이 경기를 담은 게시가 아직 날아가는 중에도 false 가 됩니다.
    // 실제로 그 경합 때문에 방금 시작한 경기의 종료가 거절됐습니다.
    await dailyPushCheckinSession();
    if(_dailyCheckinNeedsPublish){
      throw new Error('회원 화면 저장이 아직 밀려 있습니다.\n잠시 후 다시 시도해 주세요.');
    }
    const now=_dailyNow();
    const payload={
      actorPlayerId:'',
      actorPlayerName:'관리자',
      createdAt:now,
      expiresAt:now+DAILY_OFFICIAL_OPERATION_TTL_MS,
      observedUpdatedAt:now,
      source:options?.source||'system-admin-command',
      ...command,
      operationId
    };
    const callable=firebase.functions().httpsCallable('submitDailyOfficialRequest');
    const result=await callable({checkinId:_dailyCheckinId,grantToken:_dailyAdminGrantToken,command:payload});
    const receipt=result?.data||{};
    if(receipt.status!=='applied')throw new Error(receipt.reason||'현재 상태에서 처리할 수 없습니다.');
    _dailyObserveRemoteServerHead({
      revision:Number(receipt.resultRevision||0),
      lastRequestId:receipt.requestId||operationId
    });
    // 서버가 고친 결과를 그대로 받아 그립니다. 관리자가 따로 계산하지 않습니다.
    await _dailyPullServerReconcile();
    return {live:true,ok:true,receipt,operationId};
  }catch(error){
    const message=String(error?.message||'').replace(/^Firebase:\s*/,'').trim();
    if(!options?.silent)alert(message||`${label}을(를) 처리하지 못했습니다.\n네트워크를 확인한 뒤 다시 시도해 주세요.`);
    return {live:true,ok:false,reason:message};
  }
}
// 운영자가 직접 누르는 투입만 서버 명령으로 보냅니다. 재생(replay)과 자동 투입은
// 서버가 이미 정한 결과를 관리자 원본에 옮기는 일이라 그대로 로컬에서 처리합니다.
async function dailyOperatorStartQueueItem(queueId,options){
  options=options||{};
  const idx=_dailyQueue.findIndex(q=>q.id===queueId);
  const q=idx>=0?_dailyQueue[idx]:null;
  const info=q?_dailyQueueStartInfo(idx):null;
  const court=parseInt(options.court,10)||Number(info?.court)||0;
  // 서버 명령은 '빈 코트 입장 대기' 상태만 받습니다. 그 밖의 투입은 아직 로컬입니다.
  if(_dailyCheckinId&&q&&info?.state==='free'&&court&&Number(info.court)===court){
    const operationId=_dailyAdminOperationId('queue-enter');
    const sent=await _dailySendAdminCommand({
      type:'official-queue-enter-free',
      operationId,
      queueId:q.id,
      court,
      token:'dqe_'+_dailyNow().toString(36)+'_'+Math.random().toString(36).slice(2,7),
      newMatchId:`dmv2_${operationId}`,
      expectedQueueIndex:idx+1,
      expectedHoldId:info.holdId||'',
      expectedPlayerIds:[..._dailyQueueIds(q)],
      expectedTeam1Ids:[...(q.team1||[])],
      expectedTeam2Ids:[...(q.team2||[])],
      source:'system-admin-queue-enter'
    },{action:'다음 대진 투입',tag:'queue-enter'});
    return sent.ok;
  }
  return dailyStartQueueItem(queueId,options);
}
// 운영자가 누르는 종료만 서버 명령으로 보냅니다. 재생·자동 경로는 아래 로컬 함수를 씁니다.
// 승자는 싣지 않습니다 — 민턴LIVE는 승패 입력이 대진 계산에 들어가지 않습니다(2026-08-03 확정).
async function dailyOperatorCompleteMatch(id){
  const m=_dailyMatches.find(x=>x.id===id);
  if(!m||m.completedAt)return false;
  if(_dailyCheckinId){
    const sent=await _dailySendAdminCommand({
      ...
      matchId:m.id,
      court:m.court,
      token:'dct_'+_dailyNow().toString(36)+'_'+Math.random().toString(36).slice(2,7),
      expectedStartedAt:m.startedAt||0,
      expectedPlayerIds:[...(m.team1||[]),...(m.team2||[])],
      source:'system-admin-court-complete'
    },{action:'경기 종료',tag:'complete'});
    return sent.ok;
  }
  return dailyCompleteMatch(id);
}
async function dailySetTemporaryOfficial(playerId,enabled){
  ...
  try{
    ...
    const sent=await _dailySendAdminCommand({
      type:enabled?'official-temporary-grant':'official-temporary-revoke',
      operationId:_dailyTemporaryOfficialOperationId(enabled),
      playerId:player.id,
...
function _dailyApplyLiveAdditionCancel(playerId,operationAt,forcedMode){
  ...
  if(!_dailyCanCancelLiveAddition(player))return false;
  const at=Number(operationAt)||_dailyNow();
  const mode=forcedMode||(player.liveAddedOrigin==='existing'?'revert':'hide');
  _dailyCancelReservationsForPlayer(player.id,`${player.name}님의 오등록을 취소해 게임신청을 정리했습니다.`,'live-add-cancel');
  if(_dailyIsQueued(player.id)){
    const message=`${player.name}님의 오등록을 취소해 대기표를 자동 조정했습니다.`;
    if(!_dailyTryReplaceQueuedPlayer(player.id,message))_dailyRemoveQueuedPlayer(player.id,message);
  }
  if(_dailyPairSelectId===player.id)_dailyPairSelectId=null;
  ...
  player.afterMatchStatus=null;
  player.deferUntil=0;
  player.deferReason='';
  player.lastStatusAt=at;
  player.restPausedMs=0;
  player.arrivalConfirmedBy='';
  player.arrivalConfirmedByName='';
  player.arrivalConfirmedAt=0;
  player.arrivalConfirmedSource='';
  player.arrivalRequestKey='';
  _dailyClearLiveAddition(player);
  _dailyNext=null;
  dailyEnsureQueue();
  return true;
}
async function dailyCancelLiveAddition(playerId){
  if(_dailyLiveAdditionBusy)return false;
  const player=_dailyPlayer(playerId);
  if(!_dailyCanCancelLiveAddition(player)){
    alert('이미 경기에 참여했거나 진행 중인 선수는 오등록 취소할 수 없습니다.');
    return false;
  }
  const mode=player.liveAddedOrigin==='existing'?'revert':'hide';
  const action=mode==='revert'?'도착 전 상태로 되돌립니다.':'오늘 참가 명단에서 취소 처리합니다.';
  if(!confirm(`${player.name}님의 라이브 후 추가를 취소할까요?\n\n${action}`))return false;
  _dailyLiveAdditionBusy=player.id;
  dailyRenderLiveAdditions();
  try{
    const canUseServer=!!(_dailyCheckinId&&_dailyOfficialInviteHash&&typeof firebase!=='undefined'&&typeof firebase.functions==='function');
    if(canUseServer){
      if(!await dailyPushCheckinSession())throw new Error('최신 회원 화면을 저장하지 못했습니다. 잠시 후 다시 눌러 주세요.');
      if(!await _dailyEnsureAdminGrant())throw new Error('관리자 운영 연결을 확인하지 못했습니다.');
      const now=_dailyNow();
      const operationId=`dov2_add-cancel_${now.toString(36)}_${Math.random().toString(36).slice(2,12)}`;
      const command={
        type:'official-player-add-cancel',
        operationId,
        actorPlayerId:'',
        actorPlayerName:'관리자',
        playerId:player.id,
        ...
        expectedLastStatusAt:Number(player.lastStatusAt||0),
        expectedLiveAddedAt:Number(player.liveAddedAt||0),
        expectedLiveAddedOperationId:player.liveAddedOperationId||'',
        expectedGames:Number(player.games||0),
        liveAddedOrigin:player.liveAddedOrigin||'',
        candidateKey:player.liveAddedCandidateKey||'',
        createdAt:now,
        expiresAt:now+DAILY_OFFICIAL_OPERATION_TTL_MS,
        observedUpdatedAt:now,
        source:'system-admin-live-add-cancel'
      };
      const callable=firebase.functions().httpsCallable('submitDailyOfficialRequest');
      const result=await callable({checkinId:_dailyCheckinId,grantToken:_dailyAdminGrantToken,command});
      const receipt=result?.data||{};
      if(receipt.status!=='applied')throw new Error(receipt.reason||'현재 상태에서 취소할 수 없습니다.');
      _dailyObserveRemoteServerHead({
        revision:Number(receipt.resultRevision||0),
        lastRequestId:receipt.requestId||operationId
      });
      ...
      if(!_dailyApplyLiveAdditionCancel(player.id,_dailyNow(),mode))throw new Error('오등록 취소 조건이 이미 바뀌었습니다.');
      dailySave();
      dailyRender();
    }
    return true;
  }catch(error){
    alert(String(error?.message||'오등록 취소를 처리하지 못했습니다.').replace(/^Firebase:\s*/,'').trim());
    return false;
  }finally{
    _dailyLiveAdditionBusy='';
    dailyRenderLiveAdditions();
  }
}
function dailyRenderStatusBar(){
  ...
  const fair=_dailyFairSummary();
  chips.push(`<span class="daily-sb-chip ${fair.count?'warn':'on'}">${fair.count?`공정 보정 ${fair.count}명`:'공정 배정 정상'}</span>`);
...
function _dailyPersistServerIdentity(){
  try{
    ...
    const state=JSON.parse(raw);
    if(state.mode&&state.mode!=='daily'&&state.appMode!=='dailyLive')return;
    state.checkinId=_dailyCheckinId;
    state.checkinCreatedAt=_dailyCheckinCreatedAt;
    state.serverRevision=_dailyServerRevision;
    state.serverLastRequestId=_dailyServerLastRequestId;
    state.officialInviteToken=_dailyOfficialInviteToken;
    state.officialInviteHash=_dailyOfficialInviteHash;
    state.clientStateRevision=_dailyClientStateRevision;
    state.checkinNeedsPublish=_dailyCheckinNeedsPublish;
    localStorage.setItem(DAILY_KEY,JSON.stringify(state));
  }catch(e){}
}
function _dailyPersistCheckinIdentity(){
  try{
    if(_dailyCheckinId){
      ...
      else localStorage.removeItem(DAILY_CHECKIN_CREATED_KEY);
    }else{
      localStorage.removeItem(DAILY_CHECKIN_KEY);
      localStorage.removeItem(DAILY_CHECKIN_CREATED_KEY);
    }
...
function _dailyIdentitySnapshot(){
  return {
    ...
    epoch:_dailyCapabilityEpoch
  };
}
function _dailyIdentityCurrent(identity){
  return !!identity
    &&_dailyCheckinId===identity.id
    &&_dailyOfficialInviteToken===identity.token
    &&_dailyOfficialInviteHash===identity.hash
    &&_dailyCapabilityEpoch===identity.epoch;
}
function _dailyStoredIdentity(checkinId){
  try{
    const state=JSON.parse(localStorage.getItem(DAILY_KEY)||'null');
    if(!state||String(state.checkinId||'')!==String(checkinId||''))return null;
    return {
      createdAt:Math.max(0,Number(state.checkinCreatedAt||0)),
      serverRevision:Math.max(0,Number(state.serverRevision||0)),
      serverLastRequestId:String(state.serverLastRequestId||''),
      token:String(state.officialInviteToken||''),
      hash:String(state.officialInviteHash||''),
      clientStateRevision:Math.max(0,Number(state.clientStateRevision||0)),
      checkinNeedsPublish:!!state.checkinNeedsPublish
    };
  }catch(e){
    return null;
  }
...
async function _dailyLocalCapabilityValid(identity){
  const expected=identity||_dailyIdentitySnapshot();
  if(!expected.token||!expected.hash)return false;
  try{
    return await _dailyCapabilityDigest(expected.token)===expected.hash;
  }catch(e){
    return false;
  }
}
async function _dailyEnsureOfficialCapability(){
  ...
  const identityId=_dailyCheckinId;
  const capabilityEpoch=_dailyCapabilityEpoch;
  const promise=(async()=>{
    try{
      let token=_dailyOfficialInviteToken;
      let hash=_dailyOfficialInviteHash;
      if(token&&hash){
        const currentHash=await _dailyCapabilityDigest(token);
        if(_dailyCheckinId!==identityId||_dailyCapabilityEpoch!==capabilityEpoch)return false;
        if(currentHash&&currentHash===hash)return true;
        token='';
        hash='';
      }
      if(!token)token=_dailyCapabilityToken();
      if(!token)return false;
      hash=await _dailyCapabilityDigest(token);
      if(_dailyCheckinId!==identityId||_dailyCapabilityEpoch!==capabilityEpoch)return false;
      if(!hash)return false;
      _dailyOfficialInviteToken=token;
  ...
  _dailyCapabilityPromise=promise;
  try{return await promise;}
  finally{if(_dailyCapabilityPromise===promise)_dailyCapabilityPromise=null;}
}
...
function _dailyDetachStaleCheckinIdentity(){
  const detachedId=_dailyCheckinId;
  if(typeof _dailyStopOperatorHeartbeat==='function')_dailyStopOperatorHeartbeat();
  _dailyStopCheckinListener();
  _dailyClearAdminGrant();
  _dailyCheckinId=null;
  ...
  try{
    const storedId=String(localStorage.getItem(DAILY_CHECKIN_KEY)||'');
    clearSharedIdentity=!storedId||storedId===String(detachedId||'');
    if(clearSharedIdentity){
      localStorage.removeItem(DAILY_CHECKIN_KEY);
      localStorage.removeItem(DAILY_CHECKIN_CREATED_KEY);
    }
  }catch(e){}
  if(clearSharedIdentity)_dailyPersistServerIdentity();
  return detachedId;
}
function _dailyRemoteCheckinOwnership(session,localCapabilityValid,identity){
  if(session===null||session===undefined)return 'missing';
  if(!session||typeof session!=='object')return 'mismatch';
  if(localCapabilityValid===false)return 'mismatch';
  const expected=identity||_dailyIdentitySnapshot();
...
async function _dailyReadCheckinOwnership(){
  if(!_dailyCheckinId||!_fbDb)return {status:'missing',session:null};
  const identity=_dailyIdentitySnapshot();
  try{
    const snapshot=await _fbDb.ref(_dailyCheckinPath()).once('value');
    const root=snapshot.val();
    if(!_dailyIdentityCurrent(identity))return {status:'unknown',session:null};
    if(root===null||root===undefined)return {status:'missing',session:null};
    if(!root||typeof root!=='object'||!root.session||typeof root.session!=='object'){
      return {status:'mismatch',session:null};
    }
    const session=root.session;
    const localCapabilityValid=await _dailyLocalCapabilityValid(identity);
    if(!_dailyIdentityCurrent(identity))return {status:'unknown',session:null};
    return {
      status:_dailyRemoteCheckinOwnership(session,localCapabilityValid,identity),
      session
    };
  }catch(e){
...
function _dailyPullServerReconcile(retriedGrant){
  ...
  if(!_dailyCheckinId||!_dailyOfficialInviteHash)return Promise.resolve(false);
  let task;
  task=(async()=>{
    ...
    try{
      ...
      const result=await callable({
        checkinId:_dailyCheckinId,
        grantToken:_dailyAdminGrantToken,
        sinceRevision:_dailyServerRevision,
        lastRequestId:_dailyServerLastRequestId
      });
      const data=result?.data||{};
      const serverRevision=Math.max(0,Number(data.serverRevision||0));
      const serverLastRequestId=String(data.serverLastRequestId||'');
      if(serverRevision<_dailyServerRevision){
        _dailyServerReconcileError='관리자 원본이 서버 운영 기록보다 앞서 있어 자동 게시를 멈췄습니다.';
        return false;
      }
      const sameRevisionMismatch=serverRevision===_dailyServerRevision
        &&serverLastRequestId
        &&serverLastRequestId!==_dailyServerLastRequestId;
      if(serverRevision===_dailyServerRevision&&!sameRevisionMismatch){
        if(serverLastRequestId){
          _dailyServerLastRequestId=serverLastRequestId;
          _dailyPersistServerIdentity();
        }
        _dailyServerReconcileError='';
      ...
      if(sameRevisionMismatch){
        const latestIncluded=(data.commands||[]).some(req=>
          req&&req.serverAppliedAt&&Number(req.serverRevision||0)===serverRevision
        ...
        _dailyServerRevision=Math.max(0,serverRevision-1);
      }
  ...
  const wrapped=task.finally(()=>{
    if(_dailyServerSyncPromise===wrapped)_dailyServerSyncPromise=null;
  });
...
function _dailyAdoptServerSnapshot(remote,options){
  ...
  _dailyCourtOrder=_dailyNormalizeCourtOrder(remote?.event?.operatingCourtIds,remoteCourts);
...
function _dailyServerRuntimePayload(){
  ...
  _dailyMatches.forEach(match=>{
    if(!match?.completedAt||match.cancelledAt)return;
    const fourKey=_dailyMatchFourKey(match);
    const exactKey=_dailyMatchExactKey(match);
    if(fourKey)fourCounts[fourKey]=(fourCounts[fourKey]||0)+1;
    if(exactKey)exactCounts[exactKey]=(exactCounts[exactKey]||0)+1;
  });
  return {holds,nextSeq:_dailySeq,fourCounts,exactCounts};
}
function _dailyServerPlayerHistory(){
  const history=new Map();
  const row=id=>{
    if(!history.has(id))history.set(id,{partnerCountById:{},opponentCountById:{}});
    return history.get(id);
  };
  const inc=(map,id)=>{if(id)map[id]=(map[id]||0)+1;};
  _dailyMatches.forEach(match=>{
    if(!match?.completedAt||match.cancelledAt)return;
    const teams=_dailyMatchTeams(match);
    if(teams.t1.length!==2||teams.t2.length!==2)return;
    inc(row(teams.t1[0].id).partnerCountById,teams.t1[1].id);
    inc(row(teams.t1[1].id).partnerCountById,teams.t1[0].id);
    inc(row(teams.t2[0].id).partnerCountById,teams.t2[1].id);
    inc(row(teams.t2[1].id).partnerCountById,teams.t2[0].id);
    teams.t1.forEach(first=>teams.t2.forEach(second=>{
      inc(row(first.id).opponentCountById,second.id);
      inc(row(second.id).opponentCountById,first.id);
    }));
  });
  return history;
}
...
function _dailyRemoveOwnedCheckin(path,identity,requireExpired,expectedPublishId){
  ...
  return _fbDb.ref(path).transaction(root=>{
    ...
    return null;
  },undefined,false).then(result=>!!result?.committed).catch(()=>false);
}
function _dailyExpireCheckinLink(silent){
  ...
  const identity=_dailyIdentitySnapshot();
  if(_dailyCheckinOwnershipVerified)_dailyRemoveOwnedCheckin(path,identity,true);
  _dailyClearAdminGrant();
  _dailyCheckinId=null;
  _dailyCheckinCreatedAt=0;
  _dailyCheckinRequests=[];
...
async function _dailySyncArrivalCandidates(){
  ...
  // 시작된 전체 게시 트랜잭션이 그것을 다른 링크로 오인할 수 있습니다.
  if(_dailyServerSyncBusy)return true;
  try{
    const fresh=_dailyOfficialArrivalCandidates();
    const club=String(_dailyOfficialArrivalRoster()?.name||'');
    const hash=`${club}|${_dailyArrivalCandidatesHash(fresh)}`;
    if(hash===_dailyArrivalCandidatesSyncedHash)return true;
    await _fbDb.ref(_dailyCheckinPath()+'/session/arrivalClub').set(club);
...
function _dailyCheckinPayload(){
  ...
  return {
    ...
    players:_dailyPlayers
      ...
      .map(p=>{
        const history=serverPlayerHistory.get(p.id)||{partnerCountById:{},opponentCountById:{}};
        return {
        id:p.id,
        memberId:p.memberId||'',
        name:p.name,
        ...
        registrationCancelled:p.registrationCancelled===true,
        lastStatusAt:p.lastStatusAt||0,
        isGuest:!!p.isGuest,
        isClubOfficial:!!p.isClubOfficial,
        isTemporaryOfficial:!!p.isTemporaryOfficial,
        games:p.games||0,
        fairExpected:_dailyFairExpected(p),
        mixedGames:p.mixedGames||0,
        typeTrackedGames:p.typeTrackedGames||0,
        lastPlayedSeq:p.lastPlayedSeq||0,
        partnerCountById:{...history.partnerCountById},
        opponentCountById:{...history.opponentCountById},
        team:p.team||'',
        ...
        arrivalConfirmedBy:p.arrivalConfirmedBy||'',
        arrivalConfirmedByName:p.arrivalConfirmedByName||'',
        arrivalConfirmedAt:Number(p.arrivalConfirmedAt||0),
        arrivalConfirmedSource:p.arrivalConfirmedSource||'',
        liveAddedAt:Number(p.liveAddedAt||0),
        liveAddedBy:p.liveAddedBy||'',
        liveAddedByName:p.liveAddedByName||'',
        liveAddedSource:p.liveAddedSource||'',
        liveAddedOrigin:p.liveAddedOrigin||'',
        liveAddedCandidateKey:p.liveAddedCandidateKey||'',
        liveAddedOperationId:p.liveAddedOperationId||'',
        voteLocked:false,
...
function dailyEnsureCheckinId(forceNew){
  if(forceNew)_dailyDetachStaleCheckinIdentity();
  if(!_dailyCheckinId){
  ...
  if(_dailyCheckinOwnershipVerified&&_dailyCheckinExpired())_dailyExpireCheckinLink(true);
  if(!_dailyCheckinId){
    _dailyClearAdminGrant();
...
function _dailyScheduleCheckinPublishRetry(delay){
  ...
  _dailyCheckinNeedsPublish=true;
  _dailyPersistServerIdentity();
  if(!_fbDb||_dailyCheckinIdentityPending)return true;
  if(_dailyCheckinPublishRetryId)return true;
...
function _dailyMarkCheckinPublishConnected(publishedClientRevision){
  if(_dailyCheckinPublishRetryId)clearTimeout(_dailyCheckinPublishRetryId);
  _dailyCheckinPublishRetryId=null;
  _dailyCheckinPublishRetryDelay=1200;
  const publishedRevision=Math.max(0,Number(publishedClientRevision||0));
  if(publishedRevision>=_dailyClientStateRevision){
    _dailyCheckinNeedsPublish=false;
    _dailyServerReconcileError='';
  }else{
    _dailyCheckinNeedsPublish=true;
...
function _dailyWriteCheckinPayload(path){
  ...
  return _fbDb.ref(path+'/session').transaction(current=>{
    ...
    const currentPresent=current!==null&&current!==undefined;
    const currentExists=currentPresent&&typeof current==='object';
    const remoteInviteHash=String(current?.officialInvite?.tokenHash||'');
    const remoteSessionId=String(current?.serverSessionId||'');
    const remoteExpiresAt=Math.max(0,Number(current?.expiresAt||0));
    ...
    if(currentExists&&(
      ...
      ||remoteSessionId!==identity.id
      ||(remoteExpiresAt&&_dailyNow()>=remoteExpiresAt)
    ))return;
    ...
    if(wrapped.committed){
      ...
      if(Array.isArray(payload?.arrivalCandidates)){
        _dailyArrivalCandidatesSyncedHash=`${String(payload.arrivalClub||'')}|${_dailyArrivalCandidatesHash(payload.arrivalCandidates)}`;
      }
      return wrapped;
    }
...
async function _dailyPushCheckinSessionOnce(){
  if(!_dailyCheckinId||!_fbDb||_dailyCheckinIdentityPending)return false;
  if(_dailyCheckinExpired()){
    _dailyExpireCheckinLink(true);
    dailyRenderCheckinRequests();
    return false;
  }
  if(!_dailyOfficialInviteToken||!_dailyOfficialInviteHash){
    const capabilityReady=await _dailyEnsureOfficialCapability().catch(()=>false);
    if(!capabilityReady||!_dailyCheckinId||_dailyCheckinIdentityPending)return false;
  }
  const path=_dailyCheckinPath();
  const pushIdentity=_dailyIdentitySnapshot();
  try{
    const result=await _dailyWriteCheckinPayload(path);
    if(result?.identityStale||!_dailyIdentityCurrent(result?.identity))return false;
    if(result?.committed){
...
function dailyPushCheckinSession(){
  ...
  _dailyCheckinPublishQueued=true;
  if(_dailyCheckinPublishPromise)return _dailyCheckinPublishPromise;
  let publishing;
  publishing=(async()=>{
    let published=false;
    while(_dailyCheckinPublishQueued){
      if(!_dailyCheckinId||!_fbDb||_dailyCheckinIdentityPending)return false;
      _dailyCheckinPublishQueued=false;
      published=await _dailyPushCheckinSessionOnce();
      if(!published)break;
    }
    return published;
  })().finally(()=>{
    if(_dailyCheckinPublishPromise===publishing)_dailyCheckinPublishPromise=null;
    if(_dailyCheckinPublishQueued&&_dailyCheckinId&&_fbDb&&!_dailyCheckinIdentityPending){
      Promise.resolve().then(()=>dailyPushCheckinSession());
    }
  });
  _dailyCheckinPublishPromise=publishing;
  return publishing;
}
async function dailyPublishCheckinSession(silent){
  ...
  if(sharedCheckinId&&sharedCheckinId!==_dailyCheckinId){
    await dailyResumeCheckin();
    const currentSharedId=localStorage.getItem(DAILY_CHECKIN_KEY)||null;
    if(currentSharedId&&currentSharedId!==_dailyCheckinId){
      if(!silent)alert('다른 화면에서 회원 링크를 갱신 중입니다. 잠시 후 다시 눌러 주세요.');
      return null;
    }
  }
  let rotatedAfterWrite=false;
  for(let attempt=0;attempt<4;attempt++){
    dailyEnsureCheckinId();
    const ownershipResult=await _dailyReadCheckinOwnership();
    const ownership=ownershipResult.status;
    if(ownership==='unknown'){
      _dailyServerReconcileError='회원 링크 서버 연결을 확인하지 못했습니다.';
      dailyRenderCheckinRequests();
      if(!silent)alert('회원 링크 서버 연결을 확인하지 못했습니다. 네트워크를 확인한 뒤 다시 눌러 주세요.');
      return null;
    }
    ...
    if(ownership==='missing'&&_dailyCheckinExpired()){
      dailyEnsureCheckinId(true);
      continue;
    }
    ...
    _dailyPersistCheckinIdentity();
    if(!await _dailyEnsureOfficialCapability()){
      _dailyServerReconcileError='회원 링크 운영 인증을 만들지 못했습니다.';
      dailyRenderCheckinRequests();
      if(!silent)alert('회원 링크 운영 인증을 만들지 못했습니다. 보안 연결을 지원하는 브라우저에서 다시 시도해 주세요.');
      return null;
    }
    const id=_dailyCheckinId;
    const path=_dailyCheckinPath();
    const publishIdentity=_dailyIdentitySnapshot();
    let result;
    try{
      result=await _dailyWriteCheckinPayload(path);
      const resultIdentity=result?.identity||publishIdentity;
      if(result?.identityStale||!_dailyIdentityCurrent(resultIdentity))continue;
      if(!result?.committed&&_dailyRemoteCheckinOwnership(result?.snapshot?.val(),true,resultIdentity)==='owned'){
      ...
      if(!result?.committed&&_dailyRemoteCheckinOwnership(result?.snapshot?.val()||ownershipResult.session,true,resultIdentity)==='owned'){
        _dailyServerReconcileError='임원 운영 상태를 다시 불러오는 중입니다.';
        _dailyScheduleServerReconcile();
      }
    }catch(error){
    ...
    const failedOwnership=_dailyRemoteCheckinOwnership(result?.snapshot?.val(),true,resultIdentity);
    if(!rotatedAfterWrite&&(failedOwnership==='mismatch'||failedOwnership==='expired')){
      rotatedAfterWrite=true;
      dailyEnsureCheckinId(true);
      continue;
    }
...
async function dailyResumeCheckin(){
  ...
  if(storedCheckinId&&storedCheckinId!==_dailyCheckinId){
    if(typeof _dailyStopOperatorHeartbeat==='function')_dailyStopOperatorHeartbeat();
    _dailyStopCheckinListener();
    _dailyClearAdminGrant();
    _dailyCapabilityPromise=null;
    ...
    if(!storedIdentity?.token||!storedIdentity.hash){
      _dailyServerReconcileError='다른 화면에서 회원 링크를 갱신 중입니다.';
      dailyRenderCheckinRequests();
      return false;
    }
    _dailyCheckinId=storedCheckinId;
    _dailyCheckinCreatedAt=parseInt(localStorage.getItem(DAILY_CHECKIN_CREATED_KEY)||'0',10)||storedIdentity.createdAt||0;
    ...
    _dailyClientStateRevision=Math.max(_dailyClientStateRevision,storedIdentity.clientStateRevision);
    _dailyCheckinNeedsPublish=_dailyCheckinNeedsPublish||storedIdentity.checkinNeedsPublish;
    _dailyCheckinPublishQueued=_dailyCheckinPublishQueued||_dailyCheckinNeedsPublish;
    _dailyServerReconcileError='';
  ...
  dailyStartCheckinListener();
  _dailyStartOperatorHeartbeat();
  if(_dailyCheckinNeedsPublish||_dailyCheckinPublishQueued){
    const published=await dailyPushCheckinSession();
    if(!published&&_dailyCheckinNeedsPublish)_dailyScheduleCheckinPublishRetry();
  }
  if(ownership==='owned'&&_dailyOfficialInviteHash)_dailyScheduleServerReconcile();
  return true;
}
...
function _dailyQueueFromServerSyncItem(item){
  const team1=[...(item?.t1Ids||item?.team1||[])].map(String).filter(Boolean);
  const team2=[...(item?.t2Ids||item?.team2||[])].map(String).filter(Boolean);
  const ids=[...team1,...team2];
  if(ids.length!==4||new Set(ids).size!==4||ids.some(id=>!_dailyPlayer(id)))return null;
  const q={
    id:String(item?.queueId||item?.id||('dq_server_'+_dailyNow())),
    ...
    yieldedAt:Number(item?.yieldedAt||0),
    yieldedBy:item?.yieldedBy||'',
    yieldedSource:item?.yieldedSource||'',
    yieldedCount:Number(item?.yieldedCount||0),
    yieldedFromIndex:Number(item?.yieldedFromIndex||0),
    yieldedToIndex:Number(item?.yieldedToIndex||0),
    yieldedSteps:Number(item?.yieldedSteps||0),
    restPass:item?.restPass&&typeof item.restPass==='object'?JSON.parse(JSON.stringify(item.restPass)):null,
    restPassText:item?.restPassText||'',
    serverGenerated:!!item?.serverGenerated,
    serverGeneratedAt:Number(item?.serverGeneratedAt||0),
    serverGeneratedBy:item?.serverGeneratedBy||'',
    strict:item?.strict!==false,
    score:Number(item?.score||0),
    serverRestored:true
  };
  _dailyRecalcQueueItem(q);
  q.strict=item?.strict!==false&&!q.flexible;
  return q;
}
...
function _dailyPrepareServerQueueRequest(req){
  ...
  if(idx>=0&&requestFingerprint&&_dailyOfficialTeamFingerprint(_dailyQueue[idx].team1,_dailyQueue[idx].team2)!==requestFingerprint){
    _dailyQueue.splice(idx,1);
    idx=-1;
  }
...
function _dailyServerOperationAlreadyApplied(req){
  ...
  if(req.type==='official-roster-setup'){
    const rows=req.serverResult?.rosterSetup?.players;
    return !!(Array.isArray(rows)&&rows.length&&rows.every(row=>{
      const p=_dailyPlayer(row?.id);
      return p&&String(p.status||'')===String(row.status||'')&&Number(p.lastStatusAt||0)===Number(row.lastStatusAt||0);
    }));
  }
  if(req.type==='official-player-add-cancel'){
    const mode=req.serverResult?.playerAddCancel?.mode||((req.liveAddedOrigin||'')==='existing'?'revert':'hide');
    const p=_dailyPlayer(req.playerId);
    return !!(p
      &&['invited','planned'].includes(_dailyNormalizeStatus(p.status))
      &&!Number(p.liveAddedAt||0)
      &&(mode!=='hide'||p.registrationCancelled===true));
  }
  if(['official-player-status','member-player-status'].includes(req.type)){
    const p=_dailyPlayer(req.playerId);
    if(!p)return false;
  ...
  if(req.type==='official-active-yield'){
    ...
    if(req.serverResult?.drainingCourtClosed){
      const deferredId=String(req.serverResult?.deferred?.queueId||'');
      const court=Number(req.serverResult.drainingCourtClosed.court||req.court||0);
      return !!(cancelled&&Number(cancelled.cancelledAt||0)===operationAt
        &&deferredId&&_dailyQueue.some(item=>String(item.id||'')===deferredId)
        &&!_dailyMatches.some(match=>!match.completedAt&&!match.cancelledAt&&Number(match.court)===court));
    }
  ...
  if(req.type==='official-active-replace'){
    const m=_dailyMatches.find(x=>String(x.id)===String(req.matchId));
    return !!(m&&[...(m.team1||[]),...(m.team2||[])].some(id=>String(id)===String(req.inPlayerId||'')));
  }
  if(['official-temporary-grant','official-temporary-revoke'].includes(req.type)){
    const p=_dailyPlayer(req.playerId);
    return !!(p&&!!p.isTemporaryOfficial===!!req.serverResult?.temporaryOfficial?.enabled);
  }
  return false;
}
function _dailyOfficialRequestError(req){
  const actor=_dailyPlayer(req.actorPlayerId);
  const temporaryRoleCommand=['official-temporary-grant','official-temporary-revoke'].includes(req.type);
  const rosterSetupCommand=req.type==='official-roster-setup';
  // 관리자가 보낸 명령은 actorPlayerId 가 비어 있습니다(관리자는 명단에 선수로 없음).
  // 서버가 이미 적용한 것이면 임원 본인 확인 대신 그 자격으로 받습니다.
  // 서버 validateCommon 의 adminClaim 과 같은 규칙이어야 합니다 — 한쪽만 넓히면
  // 서버는 통과시킨 명령을 관리자 원본이 다시 거절해 화면이 멈춥니다(2026-08-04 실측).
  const serverAdmin=!!req.serverAppliedAt&&!req.actorPlayerId;
  ...
  if(req.type==='official-player-arrival'){
    ...
    if(!Object.prototype.hasOwnProperty.call(req,'expectedStatus')||!['invited','planned'].includes(String(req.expectedStatus||'')))return '선수의 도착 전 상태를 다시 확인해야 합니다.';
  ...
  if(req.type==='official-roster-setup'){
    const rows=req.serverResult?.rosterSetup?.players;
    if(!req.serverAppliedAt)return '오늘 명단 일괄 설정은 서버에서 확인한 뒤 적용합니다.';
    if(!Array.isArray(rows)||!rows.length)return '서버에서 확정한 오늘 명단을 확인하지 못했습니다.';
    return '';
  }
  ...
  if(req.type==='official-player-add-cancel'){
    const p=_dailyPlayer(req.playerId);
    if(!p)return '취소할 선수를 찾지 못했습니다.';
    if(!_dailyCanCancelLiveAddition(p))return '이미 경기에 참여했거나 진행 중인 선수는 오등록 취소할 수 없습니다.';
    if(String(req.expectedStatus||'')!==String(p.status||''))return '선수 상태가 이미 바뀌었습니다.';
    if(!Object.prototype.hasOwnProperty.call(req,'expectedLiveAddedAt')||Number(req.expectedLiveAddedAt)!==Number(p.liveAddedAt||0))return '라이브 후 추가 기록이 이미 바뀌었습니다.';
    if(String(req.expectedLiveAddedOperationId||'')!==String(p.liveAddedOperationId||''))return '라이브 후 추가 작업이 이미 바뀌었습니다.';
    if(!Object.prototype.hasOwnProperty.call(req,'expectedLastStatusAt')||Number(req.expectedLastStatusAt)!==Number(p.lastStatusAt||0))return '선수 상태가 이미 바뀌었습니다.';
    if(Number(req.expectedGames||0)!==Number(p.games||0))return '선수 경기 기록이 이미 바뀌었습니다.';
    return '';
  }
  ...
  if(req.type==='official-active-yield'){
    ...
    if(!req.serverResult?.deferred||(!req.serverResult?.autoEnter&&!req.serverResult?.drainingCourtClosed))return '서버의 대진 처리 결과를 다시 확인해야 합니다.';
    return '';
  }
  if(req.type==='official-queue-enter-free'){
    ...
    if(_dailyOfficialFingerprint(req.expectedPlayerIds)!==_dailyOfficialFingerprint(ids))return '다음 대진 선수가 이미 바뀌었습니다.';
    if(!_dailyOfficialTeamFingerprint(req.expectedTeam1Ids,req.expectedTeam2Ids)||_dailyOfficialTeamFingerprint(req.expectedTeam1Ids,req.expectedTeam2Ids)!==_dailyOfficialTeamFingerprint(q.team1,q.team2))return '다음 대진 팀 구성이 이미 바뀌었습니다.';
  ...
  // 서버는 통과시킨 명령을 관리자 화면이 거절합니다(2026-08-04 실측).
  if([
...
function _dailyRecordOfficialArrival(p,req,origin){
  const actor=_dailyPlayer(req.actorPlayerId);
  ...
  _dailyMarkLiveAddition(p,{
    at:p.arrivalConfirmedAt,
    actorId:p.arrivalConfirmedBy,
    actorName:p.arrivalConfirmedByName,
    source:'club-official-arrival',
    origin:origin||'existing',
    candidateKey:req.candidateKey||'',
    operationId:req.operationId||req.key||''
  });
}
function _dailyApplyOfficialArrival(req){
  ...
  _dailyRecordOfficialArrival(p,req,'existing');
  return true;
}
function _dailyApplyOfficialPlayerAdd(req){
  const serverPlayer=req.serverResult?.playerAddition?.player||null;
  const profile=serverPlayer||_dailyOfficialArrivalRosterProfile(req.memberId);
  if(!profile||_dailyHasRosterPlayer(profile))return false;
  const p=_dailyNormalize({...profile,id:req.playerId||profile.id||undefined,status:'invited'});
  _dailyPlayers.push(p);
  _dailyApplyPlayerStatus(p,'wait',req.serverAppliedAt||req.createdAt);
  _dailyRecordOfficialArrival(p,req,'roster');
  return true;
}
function _dailyApplyOfficialPlayerAddCancel(req){
  const result=req.serverResult?.playerAddCancel||{};
  const mode=result.mode||((req.liveAddedOrigin||'')==='existing'?'revert':'hide');
  return _dailyApplyLiveAdditionCancel(req.playerId,req.serverAppliedAt||req.createdAt,mode);
}
...
// 4단계에서 새로 만든 관리자 운영 명령들. 서버가 확정한 결과를 관리자 원본에 옮깁니다.
function _dailyApplyAdminOperation(req){
  ...
  if(req.type==='official-roster-setup'){
    const rows=result.rosterSetup?.players;
    if(!Array.isArray(rows)||!rows.length)return false;
    for(const row of rows){
      const id=String(row?.id||'');
      if(!id||!row?.name)return false;
      let p=_dailyPlayer(id);
      if(!p){
        p=_dailyNormalize({...row,id,status:row.status||'planned'});
        p.id=id;
        _dailyPlayers.push(p);
      }
      ...
      p.isGuest=row.isGuest===true;
      p.isClubOfficial=row.isClubOfficial===true;
      p.isTemporaryOfficial=row.isTemporaryOfficial===true;
      p.status=_dailyNormalizeStatus(row.status||'planned');
      p.statusLabel=row.statusLabel||_dailyCheckinStatusLabel(p.status);
      p.preArrivalVisible=row.preArrivalVisible===true;
      p.registrationCancelled=row.registrationCancelled===true;
      p.locked=false;
      p.currentMatchId='';
      p.afterMatchStatus=null;
      p.joinedAt=Number(row.joinedAt||p.joinedAt||at);
      p.waitFrom=Number(row.waitFrom||p.waitFrom||at);
      p.lastStatusAt=Number(row.lastStatusAt||at);
      p.restPausedMs=Number(row.restPausedMs||0);
      [
        'arrivalConfirmedBy','arrivalConfirmedByName','arrivalConfirmedAt','arrivalConfirmedSource',
        'liveAddedAt','liveAddedBy','liveAddedByName','liveAddedSource','liveAddedOrigin',
        'liveAddedCandidateKey','liveAddedOperationId'
      ].forEach(key=>{
        const value=row[key];
        if(value!==''&&value!==0&&value!=null)p[key]=value;
        else delete p[key];
      });
    }
    _dailyNext=null;
  ...
  if(req.type==='official-court-cancel'){
    ...
    return _dailyApplyMatchCancel(m,at);
  }
  if(req.type==='official-player-remove'){
    const id=String(result.playerRemove?.playerId||req.playerId||'');
    if(!id||!_dailyPlayer(id))return false;
    _dailyCancelReservationsForPlayer(id,'관리자가 명단에서 제외해 게임신청이 취소됐습니다.','admin-remove-player');
    _dailyPlayers=_dailyPlayers.filter(x=>x.id!==id);
    _dailyQueue=_dailyQueue.filter(q=>!_dailyQueueIds(q).includes(id));
    if(_dailyPairSelectId===id)_dailyPairSelectId=null;
    _dailyNext=null;
  ...
  if(req.type==='official-player-rename'){
    const info=result.playerRename;
    const p=info?_dailyPlayer(info.playerId):null;
    if(!p||!info?.name)return false;
    const prev=p.name;
    p.name=info.name;
    _dailyRenamePlayerEverywhere(prev,info.name);
    return true;
  }
  if(req.type==='official-player-create'){
    const info=result.playerCreate;
    if(!info?.playerId||_dailyPlayer(info.playerId))return false;
    const status=String(info.status||req.status||'wait')==='planned'?'planned':'wait';
    const player=_dailyNormalize({
      id:info.playerId,
      name:info.name||req.name||'',
      grade:req.grade||'C',
      gender:req.gender||'M',
      ageGroup:req.ageGroup||'40대',
      memberId:req.memberId||'',
      club:req.club||'',
      isGuest:info.isGuest===true,
      isClubOfficial:info.isClubOfficial===true,
      status,
      preArrivalVisible:status==='planned'
    });
    ...
    if(status==='wait')_dailyMarkLiveAddition(player,{at,origin:'manual',source:'system-admin-player-create'});
    _dailyNext=null;
    return true;
  }
  if(req.type==='official-manual-match'){
    const info=result.manualMatch;
    if(!info?.matchId||_dailyMatches.some(x=>x.id===info.matchId))return false;
    const ids=(info.playerIds||[]).map(String);
    const selected=ids.map(_dailyPlayer).filter(Boolean);
    if(selected.length!==4)return false;
    const idSet=new Set(ids);
    ids.forEach(pid=>_dailyCancelReservationsForPlayer(pid,'경기 등록으로 게임신청이 정리됐습니다.','admin-manual-active'));
    _dailyQueue=_dailyQueue.filter(q=>!_dailyQueueIds(q).some(id=>idSet.has(id)));
    const previousStatuses={};
    ids.forEach(pid=>{const p=_dailyPlayer(pid);if(p)previousStatuses[pid]=p.status;});
    const seq=Number(info.seq)||_dailySeq++;
    ...
    const match={
      ...
      previousStatuses,manualStarted:true,transitionStarted:info.transition===true
    };
    _dailyMatches.push(match);
    _dailyApplyFairOpportunity(match);
  ...
  if(req.type==='official-queue-delete'){
    const queueId=String(result.queueDelete?.queueId||req.queueId||'');
    const reservationId=String(result.queueDelete?.reservationId||'');
    const before=_dailyQueue.length;
    _dailyQueue=_dailyQueue.filter(q=>String(q.id)!==queueId);
    if(reservationId){
      _dailyReservations=_dailyReservations.filter(r=>String(r.id)!==reservationId);
    }
    _dailyEmergencyEditQueueId=null;
    _dailyNext=null;
    return _dailyQueue.length<before||!!reservationId;
  }
  if(req.type==='official-queue-regenerate'){
    // 서버가 새로 짠 대기표는 queueSync 로 통째로 내려옵니다. 여기서는 자리만 비웁니다.
    const queueId=String(result.queueRegenerate?.queueId||req.queueId||'');
    _dailyQueue=_dailyQueue.filter(q=>String(q.id)!==queueId);
    if(_dailyEmergencyEditQueueId===queueId)_dailyEmergencyEditQueueId=null;
    _dailyNext=null;
    return true;
  }
  if(req.type==='official-reservation-promote'){
  ...
  if(req.type==='official-finish-mode'){
    const info=result.finishMode;
    if(!info)return false;
    _dailyFinishMode=info.finishMode===true;
    _dailyFinishStartedAt=_dailyFinishMode?Number(info.at||at):0;
    if(_dailyFinishMode){
      _dailyMarkOperationStarted();
      _dailyAutoAssign=false;
    }
...
// 서버가 확정한 운영 설정을 관리자 원본에 옮깁니다. 관리자가 스스로 계산하지 않고
// 보낸 값이 아니라 '서버가 적용한 값'(serverResult.settings)만 받습니다.
function _dailyApplyOfficialSettings(req){
  ...
  if(Object.prototype.hasOwnProperty.call(settings,'courts')){
    ...
    if(courts){
      const current=_dailyCourtCount();
      const operating=req.serverResult?.courtAdjustment?.operatingCourtIds;
      if(Array.isArray(operating))_dailyCourtOrder=_dailyNormalizeCourtOrder(operating,courts);
      else _dailyResizeCourtOrder(courts,current);
      const el=document.getElementById('dailyCourts');
      if(el)el.value=courts;
      touched=true;
    }
  }
...
function _dailyApplyTemporaryOfficial(req){
  const p=_dailyPlayer(req.playerId);
  const result=req.serverResult?.temporaryOfficial;
  if(!p||!result||String(result.playerId||'')!==String(p.id))return false;
  p.isTemporaryOfficial=!!result.enabled;
  if(p.isTemporaryOfficial){
    p.temporaryOfficialGrantedAt=Number(result.updatedAt||req.serverAppliedAt||req.createdAt||_dailyNow());
    p.temporaryOfficialGrantedBy=req.actorPlayerId||'system-admin';
    p.temporaryOfficialGrantedByName=req.actorPlayerName||'관리자';
  }else{
    p.temporaryOfficialGrantedAt=0;
    p.temporaryOfficialGrantedBy='';
    p.temporaryOfficialGrantedByName='';
  }
  return true;
}
...
function _dailyApplyOfficialActiveYield(req){
  ...
  const result=req.serverResult||{};
  const drainingCourtClosed=!!result.drainingCourtClosed;
  if(!m||!result.deferred||(!drainingCourtClosed&&!result.autoEnter))return false;
  ...
  m.cancelReason=drainingCourtClosed?'club-official-draining-court-close':'club-official-active-yield';
...
function dailyProcessCheckinRequests(){
  ...
  try{
    ...
    const finishOfficial=(req,ok,reason,stateChanged)=>{
      ...
      const preserveLocalQueue=!!(
        ...
        &&['official-roster-setup','official-player-arrival','official-player-add','official-player-add-cancel','official-player-status','member-player-status','official-partner-reservation','official-partner-cancel'].includes(req.type)
      );
    ...
    _dailyCheckinRequests.forEach(req=>{
      const isMemberServerStatus=req.type==='member-player-status';
      if(String(req.type||'').startsWith('official-')||isMemberServerStatus){
        const serverRevision=Number(req.serverRevision||0);
        if(!req.serverAppliedAt&&_dailyOfficialInviteHash){
          autoRejected.push({...req,_completeRejectReason:isMemberServerStatus?'회원 화면에서 현재 상태를 확인한 뒤 다시 처리해 주세요.':'임원 본인 화면에서 현재 상태를 확인한 뒤 다시 처리해 주세요.'});
          return;
        }
        ...
        if(isMemberServerStatus){
          if(String(req.actorPlayerId||'')!==String(req.playerId||'')){
            finishOfficial(req,false,'본인 상태 요청 정보가 올바르지 않습니다.',false);
            return;
          }
          const ok=_dailyApplyOfficialStatus(req);
          finishOfficial(req,ok,'회원 상태를 관리자 원본에 연결하지 못했습니다.');
          return;
        }
        ...
        if(req.type==='official-player-add-cancel'){
          const ok=_dailyApplyOfficialPlayerAddCancel(req);
          finishOfficial(req,ok,'라이브 후 오등록 취소를 관리자 원본에 연결하지 못했습니다.');
          return;
        }
        if(req.type==='official-player-status'){
          const ok=_dailyApplyOfficialStatus(req);
        ...
        if(req.type==='official-settings-update'){
          const ok=_dailyApplyOfficialSettings(req);
          if(ok)changed=true;
          finishOfficial(req,ok,'운영 설정 변경을 관리자 원본에 연결하지 못했습니다.',true);
          return;
        }
        if(['official-roster-setup','official-player-remove','official-player-rename','official-player-create',
            'official-player-official',
        ...
        if(req.type==='official-active-replace'){
          const info=req.serverResult?.activeReplace||{};
          const m=_dailyMatches.find(x=>String(x.id)===String(info.matchId||req.matchId)&&!x.completedAt&&!x.cancelledAt);
          const outId=String(info.outId||req.outPlayerId||'');
          const inId=String(info.inId||req.inPlayerId||'');
          if(!m||!outId||!inId){finishOfficial(req,false,'교체 결과를 관리자 원본에 연결하지 못했습니다.');return;}
          const already=[...m.team1,...m.team2].some(id=>String(id)===inId);
      ...
      if(req.type==='queue-yield'||req.type==='queue-defer'){
        autoRejected.push({...req,_completeRejectReason:'이번만 뒤로는 클럽 임원이 처리합니다.'});
        return;
      }
    ...
    autoApplied.forEach(req=>{
      ...
      const isMemberServer=req.type==='member-player-status';
      ...
      const appliedBy=req.type==='official-roster-setup'?'club-official-roster-setup':['official-player-arrival','official-player-add'].includes(req.type)?'club-official-arrival':req.type==='official-player-add-cancel'?'club-official-add-cancel':isOfficial?'club-official-support':isMemberServer?'member-server-status':req.type==='court-complete'?'member-court-complete':req.type==='court-complete-undo'?'member-court-undo':isQueueYield?'member-queue-yield':isQueueCourt?'member-queue-court':'member-auto-reservation';
    ...
    autoRejected.forEach(req=>{
      ...
      const ignoredBy=req.type==='official-roster-setup'?'club-official-roster-setup':['official-player-arrival','official-player-add'].includes(req.type)?'club-official-arrival':req.type==='official-player-add-cancel'?'club-official-add-cancel':isOfficial?'club-official-support':isMemberServer?'member-server-status':req.type==='court-complete'?'member-court-complete':req.type==='court-complete-undo'?'member-court-undo':isQueueYield?'member-queue-yield':isQueueCourt?'member-queue-court':'member-auto-reservation';
  ...
  if(changed){
    if(!_dailyPaused&&!serverQueueSynced){
      dailyEnsureQueue();
      _dailyPromoteReadyReservations(true);
    }
    ...
    dailySave({preserveServerQueue:serverQueueSynced});
    dailyRender();
    dailyMaybeAutoAssign();
  }
}
...
async function dailyStopCheckinLink(){
  ...
  if(typeof _dailyStopOperatorHeartbeat==='function')_dailyStopOperatorHeartbeat();
  _dailyStopCheckinListener();
  if(_dailyCheckinOwnershipVerified)await _dailyRemoveOwnedCheckin(path,_dailyIdentitySnapshot(),false);
  _dailyClearAdminGrant();
  localStorage.removeItem(DAILY_CHECKIN_KEY);
  localStorage.removeItem(DAILY_CHECKIN_CREATED_KEY);
  _dailyCheckinId=null;
...
function dailyRender(){
  ...
  if(list){
    ...
    if(!_dailyPlayers.length)list.innerHTML='<div class="daily-empty">명단이 비어 있습니다. 「참가자 등록」에서 명부 회원을 고르거나 게스트를 추가하세요.</div>';
      ...
      const rows=sorted.length?sorted.map(p=>`<div class="daily-player ${_dailyPairSelectId===p.id?'daily-pair-selecting':''} ${_dailyReservationOfPlayer(p.id)?'daily-paired':''}">
        <span class="daily-player-main">
          <span class="daily-player-name">${_dailyNameHtml(p)}${_dailyGuestRenameButton(p)}${_dailyPlayerRemoveButton(p)} ${_dailyStatusBadge(p.status)} ${_dailyQueueLabelForPlayer(p.id)}</span>
...
function dailyRenderMatches(){
  ...
  const renderRecentMatch=m=>{
    ...
    return `<div class="daily-match">
      ...
      ${done?'':`<div class="daily-win-actions"><button class="daily-mini-btn primary-action" data-daily-complete="${m.id}" ${pausedAttr} onclick="dailyOperatorCompleteMatch('${m.id}')">${esc(_dailyCompleteButtonText(m))}</button></div>`}
    </div>`;
  };
  ...
  const renderCourt=c=>{
    ...
    if(!m){
      ...
      return `<div class="daily-court-card free">
        ...
        <div class="daily-court-body">
          ...
          ${canStart?`<div class="daily-court-actions single"><button class="daily-mini-btn primary-action" onclick="dailyOperatorStartQueueItem('${startQueue.id}',{court:${c}})">${startLabel}</button></div>`:''}
        </div>
      </div>`;
    }
    const t1=m.team1.map(_dailyPlayer).filter(Boolean),t2=m.team2.map(_dailyPlayer).filter(Boolean);
    ...
    const drainingCourt=!operatingCourtSet.has(parseInt(m.court,10)||0);
    return `<div class="daily-court-card busy ${drainingCourt?'closing ':''}${state==='due'?'due':state==='soon'?'soon':''}" data-daily-court-card="${m.id}">
      ...
      <div class="daily-court-body">
        <div class="daily-court-teams daily-court-stack">
          <div class="daily-court-team-name">${t1.map((p,i)=>playerButton('team1',p,i)).join('<span class="daily-court-plus" aria-hidden="true">+</span>')}</div>
          <div class="daily-court-vs">vs</div>
          <div class="daily-court-team-name b">${t2.map((p,i)=>playerButton('team2',p,i)).join('<span class="daily-court-plus" aria-hidden="true">+</span>')}</div>
        </div>
  ...
  const displayCourtIds=[...new Set([...operatingCourtIds,...active.map(m=>parseInt(m.court,10)||0).filter(Boolean)])].sort((a,b)=>a-b);
  const courtHtml=`<div class="daily-court-grid">${displayCourtIds.map(renderCourt).join('')}</div>`;
...
function renderDailyImportMembers(){
  ...
  const rosterRow=m=>{
    const existing=existingByName.get(m.name);
    const checked=prevChecked.has(String(m._origIdx));
  ...
  // 기존 참가자 재노출 정리(운영자 2026-08-12): 선택해도 할 게 없는 행(이미
  // 등록·경기 중)은 목록에서 뺍니다. 숫자는 머리글 「이미 있음」으로만 남습니다.
  const enabledRoster=rosterRows.filter(canRegister);
  const body=[...enabledRoster.map(rosterRow),...extras.map(extraRow)].join('');
  ...
  </div>`+(body||`<div class="dir-empty">${plannedOnly?'도착 전 선수가 없습니다':(total?'전원 이미 등록되어 있습니다':'추가할 선수가 없습니다')}</div>`);
}
...
// 이미 명단에 있는 도착 전 선수는 official-player-arrival, 클럽 명부에서 새로
// 들어오는 선수는 official-player-add 로 갑니다. 서버가 arrivalCandidates 에서
// 같은 후보를 찾을 수 있어야 하므로 candidateKey 형식을 그대로 맞춥니다.
async function _dailyRegisterArrivalsViaServer(members,club){
  const clubName=club?.name||'';
  let sent=0,failed=0;
  const reasons=[];
  for(const m of members){
    const existing=_dailyPlayers.find(p=>p.name===m.name);
    const memberId=m.memberId||_rsvpMemberId({name:m.name,club:m.club||clubName});
    let command=null;
    if(existing&&['invited','planned'].includes(_dailyNormalizeStatus(existing.status))){
      command={
        type:'official-player-arrival',
        candidateKey:`player:${existing.id}`,
        playerId:existing.id,
        playerName:existing.name,
        status:'wait',
        expectedStatus:existing.status||'',
        expectedLastStatusAt:existing.lastStatusAt||0,
        source:'system-admin-arrival'
      };
    }else if(!existing){
      const operationId=_dailyAdminOperationId('arrival-add');
      command={
        type:'official-player-add',
        operationId,
        candidateKey:`roster:${memberId}`,
        memberId,
        playerId:`dpv2_${operationId}`,
        playerName:m.name,
        expectedName:m.name,
    ...
    if(!command)continue;   // 이미 참가 상태인 선수는 건너뜁니다
    const result=await _dailySendAdminCommand(command,{action:'참가 등록',tag:'arrival',silent:true});
    if(result.ok)sent++;
    else{
      failed++;
      if(result.reason&&!reasons.includes(result.reason))reasons.push(`${m.name}: ${result.reason}`);
    }
  }
  return {sent,failed,reasons};
}
// 도착 전 등록은 이름을 명단에 올려두기만 합니다. 임원이 도착을 확인하기 전에는
// 대진에 들어가지 않으므로 서버도 status 'planned' 로만 받습니다.
async function _dailyRegisterPreArrivalsViaServer(members,club){
  const clubName=club?.name||'';
  let sent=0,failed=0;
  const reasons=[];
  for(const m of members){
    if(_dailyPlayers.some(p=>p.name===m.name))continue;   // 이미 명단에 있으면 건너뜁니다
    const operationId=_dailyAdminOperationId('prearrival');
    const gender=_dailyGender(m.gender||'남');
    const grade=m.grade||'C';
    const result=await _dailySendAdminCommand({
      type:'official-player-create',
      operationId,
      playerId:`dpv2_${operationId}`,
      name:m.name,
      memberId:m.memberId||_rsvpMemberId({name:m.name,club:m.club||clubName}),
...
async function importDailySelected(){
  ...
  // 게시 뒤에도 같은 등록 버튼을 사용합니다. 기존 도착 전 선수는 참가로 바뀌고,
  // 새 선수는 지각 참가자로 즉시 추가됩니다.
  if(_dailyCheckinId){
    const done=await _dailyRegisterArrivalsViaServer(sel,club);
    closeDailyImportModal();
    dailyRender();
    alert(done.sent
      ? `${done.sent}명을 등록했습니다.${done.failed?`\n\n${done.failed}명은 처리하지 못했습니다: ${done.reasons.join(', ')}`:''}`
      : `새로 등록된 선수가 없습니다.${done.reasons.length?`\n\n${done.reasons.join(', ')}`:''}`);
    return;
  }
  ...
  sel.forEach(m=>{
    ...
    if(!existing){
      ...
      _dailyPlayers.push(player);
      _dailyMarkLiveAddition(player,{origin:'roster',candidateKey:`roster:${player.memberId||player.id}`,source:'system-admin-roster-add'});
      added++;
      return;
    }
    if(['invited','planned'].includes(_dailyNormalizeStatus(existing.status))){
      ...
      if(existing.isClubOfficial){
        existing.isTemporaryOfficial=false;
        existing.temporaryOfficialGrantedAt=0;
        existing.temporaryOfficialGrantedBy='';
        existing.temporaryOfficialGrantedByName='';
      }
      _dailyApplyPlayerStatus(existing,'wait');
      added++;
      reactivated++;
  ...
  if(added){
    const parts=[];
    parts.push(`${added}명을 등록했습니다.`);
    alert(`${parts.join(' ')}${reactivated?` (도착 확인 ${reactivated}명)`:''}${prunedCarryover?` (이전 클럽 명단 ${prunedCarryover}명 정리)`:''}${skipped?` (중복 ${skipped}명 제외)`:''}\n\n자유게임을 진행한 뒤 대진 게시를 눌러주세요.`);
  }else{
    alert('새로 등록된 선수가 없습니다.');
  }
}
...
function doTeamAssign(){
  ...
  location.href='team.html?v=1.10.718&from=daily';
  return;
...
function _buildLiveState(){
  ...
  return {
    kind:'dailyLive',
    appMode:'dailyLive',
    title: (isTeam? (bn2+' vs '+wn2):'민턴LIVE 대진표'),
...
function _qualityAssessment(matches,participants,settings){
  ...
  // 페어 격차 비대칭 — 합이 같아도 격차 큰 쪽이 진다 (2026-08-14, 9ZJ2VH 백테스트 4/5).
  // 나란한 격차(초심전)는 서로 상쇄되는 공정한 경기라 세지 않는다.
  const asymCount=matches.filter(m=>{
    const g1=Math.abs(effLevel(m.team1A)-effLevel(m.team1B));
    const g2=Math.abs(effLevel(m.team2C)-effLevel(m.team2D));
    return Math.abs(g1-g2)>DAILY_PARTNER_GAP_SYMMETRY_LIMIT;
  }).length;
  ...
  // 팀 나누기 점검 — 경기 잣대로는 안 잡히는 나누기 결함 (9ZJ2VH 18:7 원인).
  const splitAudit=(()=>{
    if(!settings.teamMode)return null;
    ...
    return {nB:blue.length,nW:white.length,avgGap:Math.round(avgGap*100)/100,
      lowCount:lows.length,lowStacked};
  })();
  ...
  // 나누기 결함은 경기 균형이 아무리 좋아도 상급을 줄 수 없다.
  if(splitAudit&&(splitAudit.lowStacked||splitAudit.avgGap>0.3))total=Math.min(total,84);
  const grade=total>=95?'S':total>=85?'A':total>=75?'B':total>=65?'C':'D';
  const gradeLabel={S:'완벽',A:'우수',B:'양호',C:'보통',D:'재생성 권장'}[grade];
  return {gpp,avgLD,spikes,maxLD,asymCount,evenCount,favCount,splitAudit,
    counts,pGoal,under,over,underSlots,overSlots,
...
function renderQualityDashboard(matches,participants,settings){
  ...
  const {avgLD,spikes,maxLD,asymCount,evenCount,favCount,splitAudit,
  ...
  const rows=[
    (()=>{
      ...
      if(asymCount) detail+=` · 페어 격차 비대칭 ${asymCount}경기`;
      if(splitAudit){
        detail+=` · 팀 1인당 평균 차 ${splitAudit.avgGap}`;
        if(splitAudit.lowStacked) detail+=` · ⚠ 초심 ${splitAudit.lowCount}명 한 팀 몰림`;
      }
      if(!spikes.length&&!asymCount) detail+=' · 튀는 경기 없음';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('visibilitychange',()=>{
    if(!document.hidden&&_dailyCheckinId)dailyResumeCheckin().catch(()=>{});
  });
  window.addEventListener('online',()=>{
    if(_dailyCheckinId)dailyResumeCheckin().catch(()=>{});
  });
