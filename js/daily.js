...
let _dailyManualActiveDraft={court:null,ids:[]};
...
function _dailyMinutes(ts){
  if(!ts)return 0;
  return Math.max(0,Math.floor((_dailyNow()-ts)/60000));
}
function _dailyNameHtml(p){
  if(!p)return '';
  return `<span class="daily-name-chip">${esc(p.name)}${p.isGuest?'<span class="guest-badge">G</span>':''}</span>`;
}
...
function _dailyReservationStatus(r){
  ...
  if(qIdx>=0)return {cls:'queued',text:`대기표 반영됨 #${qIdx+1}`,ready:false};
...
function _dailyPromoteReadyReservations(silent){
  ...
  for(const r of _dailyReservations){
    const forceMember=r.source==='member-request';
    const existingIdx=_dailyQueue.findIndex(q=>q.reservationId===r.id);
    if(existingIdx>=0){
      if(forceMember){
        const item=_dailyQueue.splice(existingIdx,1)[0];
        let insertAt=0;
        while(insertAt<_dailyQueue.length&&_dailyQueue[insertAt].reservationId)insertAt++;
        _dailyQueue.splice(insertAt,0,item);
        _dailyQueue=_dailyQueue.slice(0,_dailyQueueCapacity().target);
        changed=true;
      }
      continue;
    }
...
function _dailyMatchEndAt(m){
  return (m.startedAt||_dailyNow())+(m.expectedMinutes||DAILY_MATCH_MINUTES)*60000;
}
function _dailyClock(ts){
  return new Date(ts).toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit'});
}
function _dailyRemainingMinutes(m){
  return Math.max(0,Math.ceil((_dailyMatchEndAt(m)-_dailyNow())/60000));
}
...
function _dailyQueueStartInfo(idx){
  ...
  const used=new Set(active.map(m=>m.court));
  const freeCourts=[];
  for(let c=1;c<=_dailyCourtCount();c++)if(!used.has(c))freeCourts.push(c);
  if(idx<freeCourts.length){
    return {state:'free',text:`${freeCourts[idx]}코트 입장 가능`,detail:'빈 코트',court:freeCourts[idx],matchId:''};
  }
  const m=active[idx-freeCourts.length];
  ...
  if(state==='soon')return {state:'soon',text:`${m.court}코트 곧 종료`,detail:`남은 ${remain}분 · 다음 대진 준비`,court:m.court,matchId:m.id};
  return {state:'normal',text:`${m.court}코트 예상`,detail:`${_dailyClock(_dailyMatchEndAt(m))} 전후 · 변동 가능`,court:m.court,matchId:m.id};
}
function _dailyQueueStartCueHtml(idx){
  const info=_dailyQueueStartInfo(idx);
  return `<span class="daily-start-cue ${info.state}" data-daily-queue-cue="${idx}">${esc(info.text)} · ${esc(info.detail)}</span>`;
}
function dailyRefreshTimers(){
  if(_dailyAutoEndIdleRestPlayers()){
    dailyEnsureQueue();
    dailySave();
    dailyRender();
    return;
...
function _dailyAutoEndIdleRestPlayers(){
  ...
  _dailyPlayers.forEach(p=>{
    ...
    if(!since||now-since<DAILY_REST_AUTO_DONE_MS)return;
...
function _dailyNaturalAutoInfo(){
  ...
  if(operating.ended&&!_dailyTeamLocked){
    ...
    auto=true;
    operatingCourts=Math.max(1,Math.min(courts,Math.floor(Math.max(pool,eligible)/4)||1));
    phase='manual';
    label='자동 운영';
    hint=`${operatingCourts}코트 자동 투입`;
  }else if(active>0){
    auto=true;
    operatingCourts=Math.max(1,Math.min(courts,Math.floor(Math.max(pool,eligible,active*4)/4)||1));
    phase='active';
    label='자동 운영';
    hint=`진행 중 · ${operatingCourts}코트 흐름 유지`;
  }else if(pool>=DAILY_AUTO_FULL_START){
    auto=true;
    operatingCourts=Math.max(1,Math.min(courts,Math.floor(pool/4)));
    phase='run';
    label='자동 운영';
    hint=phase==='run'
      ? `${operatingCourts}코트 자동 흐름`
      : `${pool}명 시작 · ${operatingCourts}코트부터`;
    ...
    if(graceLeftMs<=0){
      ...
      phase='delayed';
      label='자연 시작';
      hint=`몸풀기 흐름 후 ${operatingCourts}코트 시작`;
    }else{
      phase='grace';
      label='몸풀기 대기';
  ...
  if(auto&&operating.closingSoon&&!_dailyTeamLocked){
    phase='closing';
    label='마감 준비';
    operatingCourts=Math.max(1,Math.min(operatingCourts,active||1));
    hint=`${operating.endTime} 종료 · 다음 대진 최소화`;
  }
...
function _dailyAvailableCourt(limit){
  const courts=_dailyCourtCount();
  ...
  for(let c=1;c<=courts;c++) if(!used.has(c)) return c;
  return null;
}
...
function _dailyBalancePolicyText(flowInfo){
  ...
  const official=Math.max(0,cap.baseGoal||0);
  const extra=Math.max(0,cap.extraGoal||0);
  const free=Math.max(0,(info.operatingCourts||0)-official);
  const parts=[`공식 ${official}`];
  if(extra)parts.push(`예정 +${extra}`);
...
function _dailyCourtRecommendation(flowInfo){
  const info=flowInfo||_dailyNaturalAutoInfo();
  if(!_dailyPlayers.length||!info.auto)return null;
  const current=_dailyCourtCount();
  const ready=_dailyEligible().length;
  let recommended=1;
  if(ready>=20)recommended=4;
  else if(ready>=16)recommended=3;
  else if(ready>=8)recommended=2;
  recommended=Math.max(1,Math.min(Math.max(current,4),recommended));
  if(recommended===current)return null;
  const expanding=recommended>current;
  return {
    current,
    recommended,
    ready,
    cls:expanding?'primary':'warn',
    title:expanding?`${recommended}코트 확대 검토`:`${recommended}코트 축소 검토`,
    desc:expanding
      ? `대기 ${ready}명 · 코트 여유가 있으면 확대해도 됩니다.`
      : `대기 ${ready}명 · 진행 중 경기는 유지하고 종료 후 줄이는 편이 안전합니다.`
  };
}
...
function dailyEnsureQueue(){
  ...
  if(changed||next.length!==_dailyQueue.length)_dailyQueue=next;
...
function _dailyLoadAsNewDay(s){
  ...
  _dailyAutoAssign=false;_dailyTeamMode=false;_dailyTeamLocked=false;
...
function _dailyApplyQueueYield(playerId,queueId,source){
  ...
  _dailyQueue.splice(idx+1,0,item);
  _dailyNext=null;
  return {ok:true};
}
    ...
    return `<div class="daily-res-card ${st.cls}">
      <div class="daily-res-head">
        ...
        <div class="daily-queue-actions">
          ${st.ready?`<button class="daily-mini-btn" onclick="dailyPromoteReservation('${r.id}')">우선 반영</button>`:''}
...
function dailyMaybeAutoRecommend(){
  if(_dailyAutoBusy)return false;
...
function dailyMaybeAutoAssign(force){
  ...
  try{
    ...
    while(_dailyAvailableCourt(limit)&&made<Math.max(1,limit)){
      dailyEnsureQueue();
      const q=_dailyQueue[0];
      if(!q)break;
      if(!dailyStartQueueItem(q.id,{silent:true,auto:true,courtLimit:limit}))break;
      made++;
    }
  }finally{
...
function _dailyMarkFourCacheDirty(){_dailyFourRepeatCache=null;}
...
function _dailyScoreMatch(m,strict){
  ...
  teams[0].forEach(a=>teams[1].forEach(b=>{score+=(a.opponentCount[b.name]||0)*68;}));
...
function _dailyManualActiveCandidates(){
  const activeIds=new Set();
  _dailyActiveMatches().forEach(m=>_dailyMatchPlayers(m).forEach(p=>activeIds.add(p.id)));
  return _dailyPlayers
    .filter(p=>p&&_dailyNormalizeStatus(p.status)==='wait'&&!p.currentMatchId&&!activeIds.has(p.id))
    .sort((a,b)=>{
      if((a.games||0)!==(b.games||0))return (a.games||0)-(b.games||0);
      if((a.waitFrom||0)!==(b.waitFrom||0))return (a.waitFrom||0)-(b.waitFrom||0);
      return a.name.localeCompare(b.name,'ko');
...
function _dailyManualActiveDefaultCourt(){
  ...
  for(let c=1;c<=Math.max(base,4);c++){
    if(!used.has(c))return c;
  }
  for(let c=Math.max(base,4)+1;c<=12;c++){
    if(!used.has(c))return c;
  }
  return 1;
}
function _dailyManualActiveCourtMax(){
  const activeMax=Math.max(0,..._dailyActiveMatches().map(m=>parseInt(m.court,10)||0));
  return Math.min(12,Math.max(4,_dailyCourtCount()+2,activeMax+1));
}
...
function dailyCreateActiveMatch(){
  const candidates=_dailyManualActiveCandidates();
  if(candidates.length<4){
    alert('게임 추가는 시작 상태 선수가 4명 이상일 때 가능합니다.');
    return;
  }
  _dailyManualActiveDraft={court:_dailyManualActiveDefaultCourt(),ids:[]};
...
function dailyToggleManualActivePlayer(id){
  ...
  if(!candidates.has(id)){
    alert('현재 시작 상태인 선수만 선택할 수 있습니다.');
    dailyRenderManualActiveModal();
    return;
  }
...
function dailyRenderManualActiveModal(){
  ...
  if(hint)hint.textContent=`선택 ${_dailyManualActiveDraft.court||'-'}코트`;
  ...
  if(grid){
    grid.innerHTML=candidates.map(p=>{
      ...
      return `<button type="button" class="daily-manual-player-btn ${on?'on':''}" onclick="dailyToggleManualActivePlayer('${_dailyManualEscape(p.id)}')">
        ...
        <span class="daily-manual-player-meta">${guest}${_dailyGenderLabel(p.gender)} · ${p.grade||'C'} · ${p.games||0}게임</span>
        ${on?`<span class="daily-manual-pick">${idx+1}</span>`:''}
      </button>`;
    }).join('') || '<div class="daily-empty">시작 상태 선수가 없습니다.</div>';
  }
  ...
  if(summary){
    ...
    summary.textContent=`${_dailyManualActiveDraft.court||'-'}코트 · ${t1} vs ${t2}`;
  }
...
function dailyConfirmManualActiveMatch(){
  ...
  removedQueues.forEach(q=>{
    if(q.reservationId)_dailyCancelReservationById(q.reservationId,'자율게임 등록으로 게임신청이 자동 취소됐습니다.','admin-manual-active');
  });
  ids.forEach(pid=>_dailyCancelReservationsForPlayer(pid,'자율게임 등록으로 게임신청이 자동 취소됐습니다.','admin-manual-active'));
...
function dailyStartQueueItem(queueId,options){
  ...
  const court=(requestedCourt&&_dailyCourtAvailable(requestedCourt,courtLimit))?requestedCourt:_dailyAvailableCourt(courtLimit);
...
function _dailyRenderQueueItem(q,idx,mode){
  ...
  const canStart=!!_dailyAvailableCourt();
  ...
  return `<div class="daily-queue-item ${reserved?'locked':'flex'}" data-daily-queue-id="${q.id}" data-daily-queue-idx="${idx}" draggable="true" ondragstart="dailyQueueDragStart(event,'${q.id}')" ondragover="dailyQueueDragOver(event,'${q.id}')" ondrop="dailyQueueDrop(event,'${q.id}')" ondragend="dailyQueueDragEnd()">
    <div class="daily-queue-head">
      <div class="daily-queue-head-main">
        ...
        <div>
          <div class="daily-queue-title">${compactTitle}${urgent&&reserved?`<span class="daily-queue-badge locked">신청</span>`:''}</div>
...
function dailyStartRecommended(options){
  ...
  return dailyStartQueueItem(q.id,options||{});
}
...
function _dailyPublicEvent(){
  ...
  const cap=_dailyQueueCapacity();
  const official=flowInfo.auto?Math.max(0,cap.baseGoal||0):0;
  const extra=flowInfo.auto?Math.max(0,cap.extraGoal||0):0;
  const free=flowInfo.auto?Math.max(0,(flowInfo.operatingCourts||0)-official):0;
  ...
  return {
    ...
    queuePolicy:{
      ...
      short:!!cap.short,
      label:_dailyBalancePolicyText(flowInfo),
      detail:policyDetail
    },
    active,
    next,
...
function dailyUndoMemberComplete(token,skipConfirm){
  ...
  if(!skipConfirm&&!confirm('방금 회원이 처리한 경기 종료를 되돌릴까요?'))return false;
...
function dailyCompleteMatch(id,winnerSide,options){
  ...
  dailyEnsureQueue();
  const autoStartOk=_dailyAutoFlowEnabled();
...
function _dailyCheckinExpiresAt(){
  ...
  return startedAt?startedAt+DAILY_CHECKIN_TTL_MS:0;
}
...
function dailyStartCheckinListener(){
  if(!_dailyCheckinId||!_fbDb||_dailyCheckinListening)return;
  _dailyCheckinListening=true;
  _fbDb.ref(_dailyCheckinPath()+'/requests').on('value',snap=>{
    const raw=snap.val()||{};
...
function _dailyCheckinBlockReason(req,p){
  ...
  if(_dailyIsLockedQueued(p.id)){
    ...
    return '게임신청 대기표에 포함되어 관리자 확인 필요';
  }
  return '';
}
...
function _dailyCheckinPendingRequests(){
  return _dailyCheckinRequests.filter(req=>{
    if(req.type==='reservation'||req.type==='court-complete'||req.type==='court-complete-undo')return false;
...
function _dailyReservationRequestError(req){
  ...
  try{
    ...
    _dailyCheckinRequests.forEach(req=>{
      ...
      if(req.type==='reservation'){
        if(_dailyReservationConsentPending(req)){
          return;
        }
        if(_dailyRegisterReservationRequest(req)){
          autoApplied.push(req);
          changed=true;
        }else{
          autoRejected.push(req);
        }
        return;
      }
    ...
    autoApplied.forEach(req=>{
      ...
      const appliedBy=req.type==='court-complete'?'member-court-complete':req.type==='court-complete-undo'?'member-court-undo':isQueueYield?'member-queue-yield':'member-auto-reservation';
      if(ref)ref.update({appliedAt:now,appliedBy}).catch(()=>{});
    });
    autoRejected.forEach(req=>{
      const ref=_dailyCheckinRequestRef(req.key);
      const reason=req._completeRejectReason||_dailyReservationRequestError(req)||'자동 등록 조건 불충족';
      const isQueueYield=req.type==='queue-yield'||req.type==='queue-defer';
      const ignoredBy=req.type==='court-complete'?'member-court-complete':req.type==='court-complete-undo'?'member-court-undo':isQueueYield?'member-queue-yield':'member-auto-reservation';
    ...
    return `<div class="daily-checkin-req pending">
      <div class="daily-checkin-req-head">
        ...
        <div class="daily-checkin-req-actions">
          <button class="daily-mini-btn" onclick="dailyApproveCheckinRequest('${req.key}')">반영</button>
...
function dailyRender(){
  const queueChanged=dailyEnsureQueue();
  ...
  if(list){
    ...
    if(!_dailyPlayers.length)list.innerHTML='<div class="daily-empty">민턴LIVE 명단이 비어 있습니다. 클럽 명부를 가져오거나 선수를 직접 추가하세요.</div>';
      ...
      const sorted=_dailySortPlayersForManage(_dailyPlayers);
      list.innerHTML=sorted.map(p=>`<div class="daily-player ${_dailyPairSelectId===p.id?'daily-pair-selecting':''}">
        <div class="daily-player-main">
          <div class="daily-player-top">
            <div class="daily-player-name">${_dailyNameHtml(p)} ${_dailyStatusBadge(p.status)} ${_dailyQueueLabelForPlayer(p.id)}</div>
            <button class="daily-mini-btn danger daily-remove-btn" onclick="dailyRemovePlayer('${p.id}')">삭제</button>
          </div>
          <div class="daily-player-meta">${_dailyGenderLabel(p.gender)} · ${esc(p.grade||'C')}급 · ${esc(p.ageGroup||'40대')} · 오늘 ${p.games||0}게임 · 대기 ${_dailyMinutes(p.waitFrom)}분</div>
        </div>
        <div class="daily-player-actions">
          ${_dailyStatusSelect(p)}
        </div>
      </div>`).join('');
    }
  }
...
function rsvpImportAttendees(){
  ...
  attendees.forEach(r=>{
    add({name:r.memberName||r.name,grade:r.grade,gender:r.gender,ageGroup:r.ageGroup,club:r.club},'member');
...
function dailyRenderMatches(){
  ...
  const renderRecentMatch=m=>{
    ...
    return `<div class="daily-match">
      ...
      ${done?'':`<div class="daily-win-actions"><button class="daily-mini-btn primary-action" data-daily-complete="${m.id}" onclick="dailyCompleteMatch('${m.id}')">${esc(_dailyCompleteButtonText(m))}</button></div>`}
    </div>`;
  };
  ...
  const renderCourt=c=>{
    ...
    if(!m){
      const canStart=firstQueue&&_dailyQueueItemValid(firstQueue,null);
      return `<div class="daily-court-card free">
        ...
        <div class="daily-court-body">
          <div class="daily-court-empty">${canStart?'1순위 대기 시작 가능':'대기 없음'}</div>
          ${canStart?`<div class="daily-court-actions single"><button class="daily-mini-btn primary-action" onclick="dailyStartQueueItem('${firstQueue.id}',{court:${c}})">1순위 경기 시작</button></div>`:''}
        </div>
      </div>`;
    }
    const t1=m.team1.map(_dailyPlayer).filter(Boolean),t2=m.team2.map(_dailyPlayer).filter(Boolean);
    const state=_dailyTimerState(m);
    const playerButton=(side,p,i)=>`<button class="daily-active-player" type="button" title="이름을 눌러 대기선수로 교체" onclick="dailyPickActiveReplacement('${m.id}','${side}',${i})">${_dailyNameHtml(p)}</button>`;
    return `<div class="daily-court-card busy ${state==='due'?'due':state==='soon'?'soon':''}" data-daily-court-card="${m.id}">
      <div class="daily-court-head">
        <div class="daily-court-title"><button class="daily-court-title-btn" type="button" title="코트 번호 변경" onclick="dailyEditActiveCourt('${m.id}')">${m.court}코트</button></div>
      ...
      <div class="daily-court-body">
        ...
        <div class="daily-court-actions single">
          <button class="daily-mini-btn danger" data-daily-complete="${m.id}" onclick="dailyCompleteMatch('${m.id}')">종료</button>
        </div>
      </div>
    </div>`;
  };
...
function doTeamAssign(){
  ...
  location.href='team.html?v=1.10.313&from=daily';
  return;
