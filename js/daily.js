...
function _dailyReservationIsMember(r){
  return !!(r&&(r.source==='member-request'||r.requestKey));
}
...
function _dailyReservationStatus(r){
  ...
  if(qIdx>=0){
    return {
      ...
      detail:_dailyReservationIsMember(r)?'파트너 신청이 반영됐어요. 순번은 그대로 유지돼요.':'대기표에 반영됐습니다.',
      ready:false
    };
  }
  ...
  if(queued.length&&_dailyReservationIsMember(r)){
    return {
      cls:'queued',
      text:'현재 대진 순번 유지',
  ...
  if(q&&_dailyReservationIsMember(r)&&_dailyQueue.length>=_dailyQueueCapacity().target){
    return {
      cls:'',
      text:'기존 대진 뒤 반영 대기',
...
function _dailyPromoteReadyReservations(silent){
  ...
  for(const r of _dailyReservations){
    ...
    if(forceMember&&_dailyTryApplyReservationToExistingQueue(r)){
      changed=true;
      continue;
    }
...
function _dailyQueueRestPassActive(q){
  ...
  return created&&_dailyNow()-created<DAILY_QUEUE_REST_PASS_MS;
}
...
function _dailyQueueStartInfo(idx){
  ...
  if(_dailyQueueRestPassActive(q)&&usableBefore<freeCourts.length){
    return {state:'hold',text:'조금 쉬고 입장 대기',detail:'다음 종료 코트 우선',court:null,matchId:''};
  }
  if(!_dailyQueueRestPassActive(q)&&usableBefore<freeCourts.length){
    const court=freeCourts[usableBefore];
    return {state:'free',text:`${court}코트 입장 가능`,detail:'빈 코트',court,matchId:''};
  }
...
function _dailyAvailableCourt(limit){
  ...
  for(const c of _dailyCourtOrderForUse(cap)) if(!used.has(c)) return c;
  return null;
}
...
function dailyEnsureQueue(){
  ...
  _dailyQueue=next;
  if(_dailyApplyReservationsToExistingQueue())changed=true;
  used.clear();
...
function dailyMaybeAutoAssign(force){
  ...
  try{
    ...
    while(_dailyAvailableCourt(limit)&&made<Math.max(1,limit)){
      dailyEnsureQueue();
      const court=_dailyAvailableCourt(limit);
...
function _dailyRegisterReservationRequest(req){
  ...
  const team1=(req.team1||[]).filter(Boolean);
  const team2=(req.mode==='match'?(req.team2||[]):[]).filter(Boolean);
  const isMember=req.source==='member-game-request'||req.source==='member-request'||!!req.playerId;
  if(!isMember)_dailyReleaseTemporaryQueueForReservationIds([...team1,...team2]);
  const reservation={
    id:'dres_'+_dailyNow().toString(36)+'_'+Math.random().toString(36).slice(2,5),
    ...
    source:isMember?'member-request':'admin-request',
    requestKey:req.key||''
  };
  _dailyReservations.push(reservation);
  if(isMember)_dailyTryApplyReservationToExistingQueue(reservation);
  return true;
}
...
function dailyProcessCheckinRequests(){
  ...
  try{
    ...
    _dailyCheckinRequests.forEach(req=>{
      ...
      if(req.type==='queue-enter-free'){
        ...
        if(reason){
          ...
          const ok=dailyStartQueueItem(req.queueId,{silent:true,court:parseInt(req.court,10),ignoreRestPass:true});
    ...
    autoApplied.forEach(req=>{
      ...
      const appliedBy=req.type==='court-complete'?'member-court-complete':req.type==='court-complete-undo'?'member-court-undo':isQueueYield?'member-queue-yield':isQueueCourt?'member-queue-court':'member-auto-reservation';
    ...
    autoRejected.forEach(req=>{
      ...
      const ignoredBy=req.type==='court-complete'?'member-court-complete':req.type==='court-complete-undo'?'member-court-undo':isQueueYield?'member-queue-yield':isQueueCourt?'member-queue-court':'member-auto-reservation';
...
function dailyRenderMatches(){
  ...
  const renderCourt=c=>{
    ...
    if(!m){
      ...
      const canStart=startQueue&&_dailyQueueItemValid(startQueue,null);
