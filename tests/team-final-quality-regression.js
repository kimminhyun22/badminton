...
const uneven=make({total:94,sBalance:22});
const balanced=make({total:92,sBalance:27});
assert(better(balanced,uneven),'Prefer meaningful balance gain over a small diversity score loss');
assert(accept(balanced,uneven),'Allow a limited total trade for balance');
assert(!accept(make({total:90,sBalance:28}),uneven),'Do not lose more than three total points per reshuffle');
assert(!accept(make({total:92,sBalance:22}),uneven),'No score trade without balance improvement');
assert(!better(make({total:98,sBalance:30,avoidableOverSlots:1}),uneven),'Do not buy balance with unfair extra appearances');
assert(!better(make({total:98,sBalance:30,asymSevereCount:1}),uneven),'Protect against severe partner asymmetry');
