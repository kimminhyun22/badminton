...
const eligibilityContext={C:core};vm.runInNewContext(eligibility+';this.check=eligible;',eligibilityContext);
const partial=[fixtures()[0],{...fixtures()[1],ageGroup:''},fixtures()[2]];
...
assert.equal(partial[1].ageGroup,'','never guess missing profile or modify original');
...
const brokenEligibility={C:core};vm.runInNewContext(eligibility.replace('members.push(m)','members.push({...m,name:"wrong"})')+';this.check=eligible;',brokenEligibility);
