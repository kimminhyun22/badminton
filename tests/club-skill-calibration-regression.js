...
const eligibility=reviewSource.slice(reviewSource.indexOf('  function eligible('),reviewSource.indexOf('  function checkRoster('));
const reviewProfile=m=>({...m,ageGroup:m.ageGroup||'40대'});
const eligibilityContext={C:core,reviewProfile};vm.runInNewContext(eligibility+';this.check=eligible;',eligibilityContext);
const partial=[fixtures()[0],{...fixtures()[1],ageGroup:'unknown'},fixtures()[2]];
const checked=eligibilityContext.check({members:partial});
assert.equal(checked.members.length,2);assert.equal(checked.issues[0].reason,'연령 확인 필요');
assert.equal(partial[1].ageGroup,'unknown','never guess unknown profile or modify original');
const legacy={...fixtures()[0]};delete legacy.ageGroup;
assert.equal(eligibilityContext.check({members:[legacy]}).members.length,1);
assert.equal(legacy.ageGroup,undefined,'legacy default is read-only');
assert.equal(core.players(checked.members)[1].name,partial[2].name,'filtered snapshots match player IDs');
assert.equal(eligibilityContext.check({members:[fixtures()[0],fixtures()[0],fixtures()[1]]}).members.length,1,'both duplicate names require checking');
const brokenEligibility={C:core,reviewProfile};vm.runInNewContext(eligibility.replace('members.push(m)','members.push({...m,name:"wrong"})')+';this.check=eligible;',brokenEligibility);
assert.throws(()=>assert.equal(brokenEligibility.check({members:partial}).members[0].name,partial[0].name));
...
(async()=>{
  ...
  const corrected=await handle(db,{...input,id:id2,answers:{[questions[0].id]:'a'}},'test',now+2000);
  assert.equal(corrected.count,5);assert.equal(corrected.answers[questions[0].id],'a');
  assert.equal(corrected.reviewedAt,now,'editing an answer must not reset quarterly review');
