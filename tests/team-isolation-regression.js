...
assert(importBody.includes("_teamConfirmDetachLiveBeforeChange('참가자 불러오기')"), '참가자 불러오기는 저장된 LIVE 복구 정보를 지우기 전에 확인해야 합니다.');
assert(importBody.indexOf("_teamConfirmDetachLiveBeforeChange('참가자 불러오기')") < importBody.indexOf('_teamResetLocalLiveState'), 'LIVE 연결 해제 확인은 실제 초기화보다 먼저 실행되어야 합니다.');
...
assert(teamJs.includes('function restoreSavedBracketAction()'), '헤더/모바일 복구 버튼은 LIVE 저장본 여부에 따라 한 번에 이어가야 합니다.');
