...
assert(indexHtml.includes('id="dailyImportTeamRosterBtn"')&&indexHtml.includes('onclick="dailyImportTeamRoster()"'),'민턴라이브 선수 영역에서 팀전 명단을 가져올 수 있어야 합니다.');
assert(teamHtml.includes('id="teamImportDailyRosterBtn"')&&teamHtml.includes('onclick="teamImportDailyRoster()"'),'팀전 참가자 영역에서 민턴라이브 명단을 가져올 수 있어야 합니다.');
assert(teamHtml.includes('id="teamImportDailyRosterModalBtn"'),'기존 명부 선택 창에서도 민턴라이브 명단을 바로 가져올 수 있어야 합니다.');
...
assert.strictEqual(
  ...
  '',
  '민턴라이브 세션용 id를 다른 서비스의 명부 memberId로 승격하면 안 됩니다.'
);
...
assert.strictEqual(snapshot.players.length,1,'업데이트 전 민턴라이브 저장 명단도 한 번에 읽을 수 있어야 합니다.');
...
assert(!Object.prototype.hasOwnProperty.call(snapshot.players[0],'team'),'청·홍팀 배정은 민턴라이브로 복사하면 안 됩니다.');
assert(!Object.prototype.hasOwnProperty.call(snapshot.players[0],'partnerName'),'팀전 파트너 지정은 민턴라이브로 복사하면 안 됩니다.');

now=4000;
bridge.clear('daily');
snapshot=bridge.load('daily');
assert.strictEqual(snapshot.players.length,0,'민턴라이브 초기화 후 이전 저장 상태가 다시 나타나면 안 됩니다.');
...
assert(teamImportSource.includes('_partners=[]'),'민턴라이브에서 팀전로 파트너 지정을 복사하면 안 됩니다.');
assert(teamImportSource.includes('isClubOfficial:!!raw.isClubOfficial'),'민턴라이브 임원 프로필을 팀전에 보존해야 합니다.');
...
assert.deepStrictEqual(teamState.players.map(player=>player.name),['민턴하나','민턴둘'],'민턴라이브 선수 명단이 팀전 참가자 목록을 정확히 교체해야 합니다.');
...
assert(dailyImportSource.includes('_dailyCheckinId||_dailyOperationStarted||_dailyMatches.length'),'민턴라이브 운영이나 회원 링크 시작 후 명단 덮어쓰기를 막아야 합니다.');
assert(dailyImportSource.includes("status:'wait'"),'가져온 선수는 출석 단계 없이 바로 민턴라이브 대진 가능 상태가 되어야 합니다.');
assert(!dailyImportSource.includes('등록 전'),'민턴라이브 가져오기 로직에 폐기된 출석 용어를 다시 넣으면 안 됩니다.');
...
assert.deepStrictEqual(dailyState.players.map(player=>player.name),['팀전하나','팀전둘'],'팀전 선수 명단이 민턴라이브 선수 목록을 정확히 교체해야 합니다.');
assert(dailyState.players.every(player=>player.status==='wait'),'복사 직후 모든 선수는 별도 출석 절차 없이 대진 가능해야 합니다.');
assert(dailyState.players.every(player=>player.team===''&&!player.partnerName),'팀·파트너 운영 정보를 민턴라이브에 남기면 안 됩니다.');
assert.deepStrictEqual(dailyState.queue,[],'이전 민턴라이브 대기표를 새 명단에 연결하면 안 됩니다.');
