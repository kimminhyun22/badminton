...
assert(indexHtml.includes('id="dailyImportTeamRosterBtn"')&&indexHtml.includes('onclick="dailyImportTeamRoster()"'),'민턴LIVE 선수 영역에서 팀전LIVE 명단을 가져올 수 있어야 합니다.');
assert(teamHtml.includes('id="teamImportDailyRosterBtn"')&&teamHtml.includes('onclick="teamImportDailyRoster()"'),'팀전LIVE 참가자 영역에서 민턴LIVE 명단을 가져올 수 있어야 합니다.');
...
assert.strictEqual(snapshot.players.length,2,'팀전LIVE 현재 참가자 명단을 저장해야 합니다.');
...
assert.deepStrictEqual(dailyState.players.map(player=>player.name),['팀전하나','팀전둘'],'팀전LIVE 선수 명단이 민턴LIVE 선수 목록을 정확히 교체해야 합니다.');
