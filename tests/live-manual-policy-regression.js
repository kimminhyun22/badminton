...
assert(!teamManual.includes('출석'), '팀전LIVE 설명서에 폐기한 출석 개념이 다시 노출되면 안 됩니다.');
assert(teamManual.includes('회원 응답 없이 관리자가 명단을 확정합니다.'), '팀전LIVE 설명서는 관리자 확정 명단 방식을 안내해야 합니다.');
assert(teamManual.includes('별도 단계 없이 진행 중인 실중계로 바로 들어갑니다.'), '팀전LIVE 설명서는 본인 확인 후 즉시 실중계로 이어짐을 안내해야 합니다.');
assert(teamManual.includes('민턴LIVE 선수 명단을 가져옵니다.'), '팀전LIVE 설명서는 서비스 간 명단 재사용을 안내해야 합니다.');
assert(teamManual.includes('관리자 앱이 꺼져 있어도 승패는 즉시 반영됩니다.'), '팀전LIVE 설명서는 임원 승패 입력이 관리자 앱과 독립적임을 안내해야 합니다.');
assert(teamManual.includes('팀전LIVE 이어가기'), '팀전LIVE 설명서는 앱 재실행 후 이어가기 동선을 안내해야 합니다.');
assert(teamManual.includes('운영 현황의 숫자를 누르면 운영진과 뒷풀이 신청 인원·이름을 확인'), '팀전LIVE 설명서는 실제 운영 현황 명단 확인 동선을 안내해야 합니다.');
assert(!teamManual.includes('92점 이상'), '팀전LIVE 설명서는 구현과 다른 고정 품질점수 기준을 안내하면 안 됩니다.');
assert(teamManual.includes('48시간까지만 이어갈 수 있습니다.'), '팀전LIVE 설명서는 중계 재개 기한과 정리 시점을 정확히 안내해야 합니다.');
assert(!teamManual.includes('뒤풀이'), '팀전LIVE 설명서의 뒷풀이 표기를 통일해야 합니다.');
