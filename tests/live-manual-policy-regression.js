...
assert(mintonManual.includes('도착 전에는 실중계만 볼 수 있고 대진에서는 제외됩니다.'), '민턴라이브 설명서는 도착 전 선수가 자동대진에서 제외됨을 안내해야 합니다.');
assert(mintonManual.includes('임원·도우미의 처리는 관리자 앱 없이도 즉시 반영됩니다.'), '민턴라이브 설명서는 임원·도우미의 독립 운영을 안내해야 합니다.');
assert(mintonManual.includes('한 순번만 미룹니다.'), '민턴라이브 설명서는 이번만 뒤로가 한 순번 이동임을 안내해야 합니다.');
assert(mintonManual.includes('휴식·종료 옆의 작은 뒷풀이 버튼'), '민턴라이브 설명서는 현재 뒷풀이 버튼 위치를 안내해야 합니다.');
assert(mintonManual.includes('라이브 후 추가') && mintonManual.includes('아직 경기하지 않은 오등록만'), '민턴라이브 설명서는 운영 시작 뒤 추가 선수의 정확한 확인·취소 범위를 안내해야 합니다.');
assert(mintonManual.includes('경기 종료·입장·이번만 뒤로는 45초 안에 취소'), '민턴라이브 설명서는 되돌릴 수 있는 운영 처리와 시간을 정확히 안내해야 합니다.');
assert(!mintonMemberGuide.includes('이번만 뒤로'), '일반회원 안내에 임원 전용 이번만 뒤로 권한을 넣으면 안 됩니다.');
assert(!mintonManual.includes('예상 대진'), '민턴라이브 설명서에 제거된 예상 대진 표현이 남으면 안 됩니다.');
assert(indexHtml.includes('<span class="card-title">다음 대진</span>') && !indexHtml.includes('<span class="card-title">다음·예상 대진</span>'), '민턴라이브 관리자 화면도 다음 대진만 간결하게 표시해야 합니다.');
...
assert(teamManual.includes('민턴라이브 선수 명단을 가져옵니다.'), '팀전 설명서는 서비스 간 명단 재사용을 안내해야 합니다.');
