...
assert(memberEventBoard.includes("lastComplete?.operation==='queue-yield'")&&memberEventBoard.includes("lastComplete?.operation==='queue-enter'"),'최근 임원 입장·순서 작업도 해당 이름으로 바로 취소할 수 있어야 합니다.');
...
assert(officialProcessSource.includes("_dailyCaptureCompleteUndo(req.token,'club-official-queue-enter')")&&officialProcessSource.includes("_dailyCaptureCompleteUndo(req.token,'club-official-queue-yield')"),'입장 처리와 이번만 뒤로도 적용 직전 상태를 45초 취소용으로 보존해야 합니다.');
