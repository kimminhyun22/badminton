...
// 2026-09-13: 네 번째 경로 — 임원의 「진행 중 코트 등록」 시트(openOfficialCourtCompose).
assert((src.match(/requestAnimationFrame\(\(\)=>\{try\{autoFitNames\(\);\}catch\(e\)\{\}\}\);/g) || []).length === 4,
  '시트를 여는 네 경로 모두 보인 다음 프레임에 이름 크기를 다시 재야 합니다(두 번 탭 문제).');
