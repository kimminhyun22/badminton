...
5. **다음 주**: 클럽 임원이 「새 운동일 시작」(`official-session-rollover`). 같은 링크·같은 명단, 전원 도착 전. 관리자 게시가 다시 필요 없다.
...
| `live/checkin_<ID>/requests/<key>` | 회원·임원 요청과 서버 결과(`serverAppliedAt`·`serverRejectedAt`·`serverResult`). 처리된 행은 약 15분 뒤 정리 | 회원·임원 화면, 서버 |
...
| `liveArchive/checkin_<ID>/<at>` | 롤오버 때 지난 운동 전문(completedLog·미종료 경기). 세션은 명령마다 통째로 트랜잭션되고 회원 전원이 구독하므로 세션 밖에 둔다 | 서버(콜러블) |

`live/<6자리>` 는 팀전이다(범위 밖). `cleanupExpiredLive`(6시간마다)가 `expiresAt` 지난 `live/*` 와 짝이 되는 `liveArchive/*` 를 지운다.
...
| `submitDailyOfficialRequest` | 임원·관리자 명령. `live/checkin_<ID>` 전체를 한 트랜잭션으로 → 래퍼 `applyCommandTransaction`(`functions/daily-official-command.js`) → 엔진 `applyOfficialRequest`(`functions/daily-official-engine.js`). 롤오버면 커밋 뒤 `liveArchive` 에 전문을 쓴다 |
...
- **상태 게이트**: 행위자가 `invited`·`planned`·`done` 이면 거절한다(도착 전·종료한 사람은 운영하지 못한다). 예외는 롤오버 하나 — 지난주 「종료」로 남은 클럽 임원이 보낼 수 있어야 해서 래퍼와 엔진 두 곳 모두 예외를 둔다.
- **관리자 명령**: 게시 뒤에는 관리자도 로컬을 직접 고치지 않고 같은 경로로 보낸다(`_dailySendAdminCommand`). 보내기 전에 먼저 게시해 서버가 모르는 경기를 만들지 않는다. 관리자 연결은 선수에 묶이지 않은 grant(`adminClaim`)라 행위자 게이트를 건너뛴다.
...
도우미와 클럽 임원의 구분은 대부분 **임원 화면**이 건다(`sendOfficial*` 안의 `isClubOfficial` 검사). 서버가 따로 클럽 임원만 받는 것은 롤오버와 도우미 지정·해제이고, 관리자만 받는 것은 임원 자격 부여다.
...
3. **채택**: `_dailyAdoptServerSnapshot`, 수동 「서버 기준으로 다시 맞추기」(`dailyAdoptServerState`). 롤오버는 재생하지 않고 항상 채택한다(`session/rolloverAt` 리스너 → `_dailyMaybeAdoptRollover`). 지난 운동은 localStorage `daily_day_archive_v1`(최근 8일)에 남긴다.
...
임원 화면의 버튼은 게시 페이로드의 능력 표시가 켜져 있어야 뜬다. 옛 관리자가 게시한 세션에서 새 버튼이 헛돌지 않게 하는 장치다. 현재 13종: `officialOpsV1` · `officialOpsServerV2` · `officialArrivalV1` · `officialAutoHandoffV1` · `officialLiveAdditionCancelV1` · `officialOperationStartV1` · `officialOperationUndoV1` · `officialPartnerOpsV1` · `officialQueueCardOpsV1` · `officialQueueHoldV1` · `officialQueueYieldV1` · `officialQueueYieldOneStepV1` · `officialSessionRolloverV1`. 새 임원 기능에 버튼을 달면 표시도 하나 늘린다.
...
| `REQUEST_DONE_RETAIN_MS` 처리된 요청 행 | 15분 | engine |
